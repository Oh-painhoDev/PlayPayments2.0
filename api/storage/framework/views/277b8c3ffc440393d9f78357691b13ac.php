<?php $__env->startSection('title', 'Astrofy - Integrações'); ?>
                ...
                <div class="flex flex-col gap-6 mb-8">
                    ...
                    <div class="flex items-start gap-6">
                        <div class="flex-shrink-0 w-16 h-16 rounded-xl p-3 bg-gradient-to-br from-gray-800/30 to-transparent">
                            <div style="color: #21b3dd; font-weight: bold; font-size: 20px; text-align: center; line-height: 48px;">Astrofy</div>
                        </div>
                        <div>
                                        ...
                                        <tbody class="[&_tr:last-child]:border-0 pt-2" id="integrationsTableBody">
                                            <?php $__empty_1 = true; $__currentLoopData = $integrations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $integration): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                                <tr class="bg-[#1F1F1F] hover:bg-[#252525] transition-colors rounded-lg" 
                                                    ...
                                                    data-integration-gateway-key="<?php echo e($integration->gateway_key ?? ''); ?>"
                                                    data-integration-base-url="<?php echo e($integration->base_url); ?>">
                                                    ...
                                                    <td class="py-3 px-6 font-['Manrope'] font-semibold text-[12px] tracking-[-0.24px] text-[#707070]">
                                                        <?php if($integration->gateway_key): ?>
                                                            <span class="text-green-400">✓ <?php echo e(substr($integration->gateway_key, 0, 20)); ?>...</span>
                                                        <?php else: ?>
                                                    ...
                                                    <td class="py-3 px-6">
                                                        <div class="flex items-center gap-2">
                                                            <?php if(!$integration->gateway_key): ?>
                                                                <button onclick="showAstrofyInfo(<?php echo e($integration->id); ?>, '<?php echo e($integration->name); ?>', '<?php echo e($integration->base_url); ?>')" class="text-[#707070] hover:text-blue-400 transition-colors" title="Ver informações para Astrofy">
                                                            ...
                                                            <form action="<?php echo e(route('integracoes.astrofy.destroy', $integration->id)); ?>" method="POST" onsubmit="return confirm('Tem certeza que deseja remover esta integração?');" class="inline">
                                                                <?php echo csrf_field(); ?>
                                            ...
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                                <tr>
                                                    <td colspan="5" class="py-8 px-6 text-center">
                                                        ...
                                                        <?php if($hasGateway): ?>
                                                            <button onclick="openModal()" class="mt-4 inline-flex items-center justify-center gap-2 text-sm font-medium text-white transition-all duration-200 h-10 px-4 py-2 rounded-xl hover:bg-[#21b3dd]/90" style="background-color: #21b3dd;">
            ...
            <form id="astrofyForm" method="POST" action="" class="mt-6 flex flex-col gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <input name="name" id="integrationName" type="text" class="flex h-10 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 w-full rounded-xl border-2 bg-[#1f1f1f] border-[#2a2a2a] text-white placeholder:text-[#707070] focus:border-[#21b3dd]" placeholder="Ex: Meu Gateway Astrofy" value="" required>
                ...
                <div class="p-4 rounded-xl bg-[#1f1f1f] border border-[#2a2a2a]">
                    ...
                    <div class="space-y-2 text-xs text-[#AAAAAA]">
                        <p><strong class="text-white">Base URL:</strong> <code class="text-[#21b3dd]"><?php echo e($baseUrl); ?></code></p>
        ...
        if (integrationData) {
            ...
            if (!gatewayKeyField) {
                ...
                gatewayKeyDiv.innerHTML = `
                    ...
                    <div class="flex flex-col gap-1.5">
                        <input name="gateway_key" id="gatewayKey" type="text" class="flex h-10 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 w-full rounded-xl border-2 bg-[#1f1f1f] border-[#2a2a2a] text-white placeholder:text-[#707070] focus:border-[#21b3dd]" placeholder="Cole o Gateway Key recebido da Astrofy" value="">
                        <p class="text-xs text-[#707070]">
            ...
            form.action = `<?php echo e(url('integracoes/astrofy')); ?>/${integrationData.id}`;
        } else {
            ...
            form.action = '<?php echo e(route("integracoes.astrofy.store")); ?>';
        }
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs1 gateway\resources\views/integracoes/astrofy/index.blade.php ENDPATH**/ ?>