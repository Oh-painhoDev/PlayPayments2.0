...
<head>
    ...
    <title>Checkout - <?php echo e($paymentLink->title); ?></title>
...
<body class="flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-[#161616] rounded-2xl border border-[#2d2d2d] p-6 shadow-2xl">
        <div class="text-center mb-6">
            <h1 class="text-2xl font-bold text-white mb-2"><?php echo e($paymentLink->title); ?></h1>
            <?php if($paymentLink->description): ?>
                <p class="text-[#AAAAAA] text-sm"><?php echo e($paymentLink->description); ?></p>
            <?php endif; ?>
        </div>

        <form id="checkoutForm" class="space-y-4">
            <?php if($paymentLink->allow_custom_amount || !$paymentLink->amount): ?>
                <div>
                    <label class="block text-sm font-semibold text-[#AAAAAA] mb-2">Valor (R$)</label>
                    <input type="text" id="amount" name="amount" required 
                           class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]"
                           placeholder="0,00" 
                           oninput="formatCurrency(this)"
                           <?php if($paymentLink->min_amount): ?> data-min="<?php echo e($paymentLink->min_amount); ?>" <?php endif; ?>
                           <?php if($paymentLink->max_amount): ?> data-max="<?php echo e($paymentLink->max_amount); ?>" <?php endif; ?>>
                    <?php if($paymentLink->min_amount || $paymentLink->max_amount): ?>
                        <p class="text-xs text-[#707070] mt-1">
                            <?php if($paymentLink->min_amount): ?> Mínimo: R$ <?php echo e(number_format($paymentLink->min_amount, 2, ',', '.')); ?> <?php endif; ?>
                            <?php if($paymentLink->min_amount && $paymentLink->max_amount): ?> • <?php endif; ?>
                            <?php if($paymentLink->max_amount): ?> Máximo: R$ <?php echo e(number_format($paymentLink->max_amount, 2, ',', '.')); ?> <?php endif; ?>
                        </p>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                <div class="bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg p-4 text-center">
                    ...
                    <p class="text-3xl font-bold text-white">R$ <?php echo e(number_format($paymentLink->amount, 2, ',', '.')); ?></p>
                </div>
            <?php endif; ?>

            <?php if($paymentLink->payment_method === 'all'): ?>
                <div>
                    <label class="block text-sm font-semibold text-[#AAAAAA] mb-2">Forma de Pagamento</label>
                    <select id="payment_method" name="payment_method" required 
                            class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd]">
            ...
            <?php else: ?>
                <input type="hidden" id="payment_method" name="payment_method" value="<?php echo e($paymentLink->payment_method); ?>">
            <?php endif; ?>
            ...
            <div class="border-t border-[#2d2d2d] pt-4 space-y-4">
                <div>
                    ...
                    <input type="text" id="customer_name" name="customer[name]" required 
                           class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]"
            ...
            <button type="submit" id="submitBtn" 
                    class="w-full bg-[#21b3dd] hover:bg-[#21b3dd] text-white font-semibold py-3 rounded-lg transition-colors">
                Finalizar Pagamento
            </button>
        </form>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button onclick="copyPixCode()" 
                                    class="px-4 py-2 bg-[#21b3dd] hover:bg-[#21b3dd] text-white rounded-lg text-sm font-semibold transition-colors">
                                Copiar
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        document.getElementById('checkoutForm').addEventListener('submit', async function(e) {
            ...
            const data = {
                amount: formData.get('amount') ? formData.get('amount').replace(/\./g, '').replace(',', '.') : '<?php echo e($paymentLink->amount); ?>',
            ...
            try {
                const response = await fetch('<?php echo e(route("checkout.process", $paymentLink->slug)); ?>', {
                    method: 'POST',
                    headers: {
...
<?php /**PATH C:\xampp\htdocs1 gateway\resources\views/payment-links/checkout.blade.php ENDPATH**/ ?>