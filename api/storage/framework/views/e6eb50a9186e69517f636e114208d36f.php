...
<?php $__env->startSection('title', 'Links de Pagamento'); ?>
        ...
        <div class="bg-[#000000] rounded-2xl space-y-6">
            ...
            <div class="content-stretch flex flex-col md:flex-row items-start md:items-center justify-start md:justify-between relative size-full p-5 gap-6 md:gap-0">
                ...
                <div class="flex gap-3">
                    <button onclick="openCreateModal()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#21b3dd] hover:bg-[#21b3dd] text-white rounded-lg text-sm font-semibold transition-colors">
            ...
            <div class="bg-[#161616] rounded-2xl border border-[#1f1f1f] overflow-hidden mx-5 mb-5">
                <?php if($paymentLinks->count() > 0): ?>
                                    ...
                                    <tr class="hover:bg-[#1f1f1f] transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex flex-col">
                                                <span class="font-semibold text-white text-sm"><?php echo e($link->title); ?></span>
                                                <?php if($link->description): ?>
                                                    <span class="text-xs text-[#AAAAAA] mt-1"><?php echo e(strlen($link->description) > 50 ? substr($link->description, 0, 50) . '...' : $link->description); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php if($link->allow_custom_amount): ?>
                                                <span class="text-sm text-[#AAAAAA]">Personalizado</span>
                                                <?php if($link->min_amount || $link->max_amount): ?>
                                                    <div class="text-xs text-[#707070] mt-1">
                                                        <?php if($link->min_amount): ?> Min: R$ <?php echo e(number_format($link->min_amount, 2, ',', '.')); ?> <?php endif; ?>
                                                        <?php if($link->max_amount): ?> Max: R$ <?php echo e(number_format($link->max_amount, 2, ',', '.')); ?> <?php endif; ?>
                                                    </div>
                                                <?php endif; ?>
                                            <?php else: ?>
                                                <span class="font-bold text-white">R$ <?php echo e(number_format($link->amount, 2, ',', '.')); ?></span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <input type="text" value="<?php echo e($link->checkout_url); ?>" readonly onclick="this.select()" class="px-3 py-1.5 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white text-xs font-mono w-[300px] focus:outline-none focus:ring-2 focus:ring-[#21b3dd]">
                                                <button onclick="copyToClipboard('<?php echo e($link->checkout_url); ?>')" class="px-3 py-1.5 bg-[#1f1f1f] hover:bg-[#2d2d2d] border border-[#2d2d2d] rounded-lg text-[#21b3dd] text-xs font-semibold transition-colors">
                                                    Copiar
                                                </button>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php if($link->is_active && $link->canBeUsed()): ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="text-sm text-white"><?php echo e($link->current_uses); ?></span>
                                            <?php if($link->max_uses): ?>
                                                <span class="text-xs text-[#707070]">/ <?php echo e($link->max_uses); ?></span>
                                            <?php else: ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <button onclick="getIntegrationCode('<?php echo e($link->slug); ?>')" class="px-3 py-1.5 bg-[#1f1f1f] hover:bg-[#2d2d2d] border border-[#2d2d2d] rounded-lg text-[#21b3dd] text-xs font-semibold transition-colors">
                                                    Código
                                                </button>
                                                <button onclick="editLink(<?php echo e($link->id); ?>)" class="px-3 py-1.5 bg-[#1f1f1f] hover:bg-[#2d2d2d] border border-[#2d2d2d] rounded-lg text-[#AAAAAA] text-xs font-semibold transition-colors">
                                                    Editar
                                                </button>
                                                <button onclick="deleteLink(<?php echo e($link->id); ?>)" class="px-3 py-1.5 bg-[#1f1f1f] hover:bg-[#ff6b6b] border border-[#2d2d2d] rounded-lg text-[#ff6b6b] text-xs font-semibold transition-colors">
                                                    Excluir
                                                </button>
                                            </div>
                                        </td>
                            ...
                            <div class="p-4">
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="text-sm text-[#AAAAAA]">
                                        ...
                                        <?php else: ?>
                                            <p>Valor: <span class="text-white font-bold">R$ <?php echo e(number_format($link->amount, 2, ',', '.')); ?></span></p>
                                        <?php endif; ?>
                                        <p>Uso: <span class="text-white"><?php echo e($link->current_uses); ?></span> <?php if($link->max_uses): ?>/ <?php echo e($link->max_uses); ?><?php else: ?>/ ∞<?php endif; ?></p>
                                    </div>
                    ...
                    <div class="px-6 py-4 bg-[#1f1f1f] border-t border-[#2d2d2d]">
                        <?php echo e($paymentLinks->appends(request()->query())->links()); ?>
                ...
                <?php else: ?>
                    <div class="p-12 text-center">
                        ...
                        <button onclick="openCreateModal()" class="px-4 py-2 bg-[#21b3dd] hover:bg-[#21b3dd] text-white rounded-lg text-sm font-semibold transition-colors">
                            Criar Primeiro Link
                        </button>
                    </div>
                <?php endif; ?>
    ...
    <div class="bg-[#161616] rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-[#1f1f1f]">
        ...
        <form id="paymentLinkForm" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="linkTitle" name="title" required class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="Ex: Ebook de Receitas Fit">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-[#AAAAAA] mb-2">Descrição do Produto</label>
                        <textarea id="linkDescription" name="description" rows="3" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="Ex: Acesso ao Ebook Receitas Fit"></textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-[#AAAAAA] mb-2">Valor (R$)</label>
                            <input type="text" id="linkAmount" name="amount" inputmode="decimal" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="0,00" oninput="formatCurrency(this)">
                        ...
                        <div>
                            ...
                            <select id="linkPaymentMethod" name="payment_method" required class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd]">
                    ...
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" id="linkAllowCustom" name="allow_custom_amount" value="1" class="w-4 h-4 rounded border-[#2d2d2d] bg-[#1f1f1f] text-[#21b3dd] focus:ring-[#21b3dd]">
                    ...
                    <div id="customAmountFields" class="hidden grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="linkMinAmount" name="min_amount" inputmode="decimal" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="0,00" oninput="formatCurrency(this)">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-[#AAAAAA] mb-2">Valor Máximo (R$)</label>
                            <input type="text" id="linkMaxAmount" name="max_amount" inputmode="decimal" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="0,00" oninput="formatCurrency(this)">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="linkMaxUses" name="max_uses" min="1" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd] placeholder-[#707070]" placeholder="Ilimitado">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" id="linkExpiresAt" name="expires_at" class="w-full px-4 py-3 bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#21b3dd]">
                        </div>
                    </div>
                </div>
                    ...
                    <div id="modalPreviewContainer" class="bg-[#1f1f1f] border border-[#2d2d2d] rounded-xl p-6">
                        <div class="text-center space-y-4">
                            ...
                            <div class="bg-[#161616] border border-[#2d2d2d] rounded-lg p-4">
                                ...
                                <p id="previewLink" class="text-xs font-mono text-[#21b3dd] break-all"><?php echo e(url('/checkout')); ?>/...</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex gap-3 pt-4 border-t border-[#2d2d2d]">
                <button type="submit" class="flex-1 bg-[#21b3dd] hover:bg-[#21b3dd] text-white px-6 py-3 rounded-lg font-semibold transition-colors">
                    Salvar Link
                </button>
    ...
    <div class="bg-[#161616] rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-[#1f1f1f]">
        ...
        <div class="p-6 space-y-4">
            <div class="bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg p-4">
                ...
                <button onclick="copyIntegrationCode()" class="mt-3 px-4 py-2 bg-[#21b3dd] hover:bg-[#21b3dd] text-white rounded-lg text-sm font-semibold transition-colors">
                    Copiar Código
                </button>
            </div>
            <div class="bg-[#1f1f1f] border border-[#2d2d2d] rounded-lg p-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="copyDirectLink()" class="px-4 py-2 bg-[#1f1f1f] hover:bg-[#2d2d2d] border border-[#2d2d2d] rounded-lg text-[#21b3dd] text-sm font-semibold transition-colors">
                        Copiar
                    </button>
                </div>
            </div>
...
function openCreateModal() {
    ...
    document.getElementById('paymentLinkForm').action = '<?php echo e(route("payment-links.store")); ?>';
...
function getIntegrationCode(slug) {
    ...
    const checkoutUrl = `<?php echo e(url('/checkout')); ?>/${slug}`;
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u999974013/domains/playpayments.com/public_html/resources/views/payment-links/index.blade.php ENDPATH**/ ?>