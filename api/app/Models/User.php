...
class User extends Authenticatable implements JWTSubject
{
     ...
     */
    public function calculateWithdrawalFee(float $requestedAmount): array
    {
        $feeAmount = 0.00;
        $totalToDebit = $requestedAmount;
        
        if ($this->withdrawal_fee_type === 'global') {
            // Use global fee
            $feeAmount = \App\Helpers\FeeHelper::getWithdrawalFee();
            $totalToDebit = $requestedAmount + $feeAmount;
        } elseif ($this->withdrawal_fee_type === 'fixed') {
            // Fixed fee only
            $feeAmount = $this->withdrawal_fee_fixed;
            $totalToDebit = $requestedAmount + $feeAmount;
        } elseif ($this->withdrawal_fee_type === 'percentage') {
            // Percentage fee only
            $feeAmount = ($requestedAmount * $this->withdrawal_fee_percentage) / 100;
            $totalToDebit = $requestedAmount + $feeAmount;
            ...
            $feeAmount = $this->withdrawal_fee_fixed + $percentageFee;
            $totalToDebit = $requestedAmount + $feeAmount;
        }
        
        return [
            'fee' => round($feeAmount, 2),
            'total_to_debit' => round($totalToDebit, 2),
            'net_amount' => round($requestedAmount, 2)
        ];
    }
