    ...
    public function receipt(string $transactionId)
    {
        try {
            ...
            if (isset($transaction->customer_data['address']) && is_array($transaction->customer_data['address'])) {
                ...
                $customerAddress['zipcode'] = $transaction->customer_data['zipcode'] ?? $transaction->customer_data['cep'] ?? '';
            } else {
                ...
                if (isset($transaction->customer_data['street']) || isset($transaction->customer_data['address_line1'])) {
                    $customerAddress = [
                        'street' => $transaction->customer_data['street'] ?? $transaction->customer_data['address_line1'] ?? '',
                        ...
                        'zipcode' => $transaction->customer_data['zipcode'] ?? $transaction->customer_data['cep'] ?? '',
                    ];
                }
            }
