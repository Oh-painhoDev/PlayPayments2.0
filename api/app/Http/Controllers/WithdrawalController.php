...
class WithdrawalController extends Controller
{
        ...
        try {
            ...
            if ($availableBalance < $totalToDebit) {
                ...
                $blockedMessage = $wallet->blocked_balance > 0 
                    ...
                    : '';
                return back()->withErrors(['amount' => 'Saldo disponível insuficiente para saque + taxa. Seu saldo disponível é R$ ' . number_format($availableBalance, 2, ',', '.') . '. Total necessário: R$ ' . number_format($totalToDebit, 2, ',', '.') . ' (Saque: R$ ' . number_format($requestedAmount, 2, ',', '.') . ' + Taxa: R$ ' . number_format($fee, 2, ',', '.') . ')' . $blockedMessage])->withInput();
            }
    ...
    {
        try {
            // Get active BaaS gateway
            $baasGateway = BaasCredential::where('is_active', true)->first();
                
            if (!$baasGateway) {
                throw new \Exception('BaaS gateway not found');
            }
