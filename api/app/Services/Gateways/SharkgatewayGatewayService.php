...
class SharkgatewayGatewayService extends BaseGatewayService
{
    ...
    public function createTransaction(User $user, array $data): array
    {
        try {
            ...
            if (isset($data['shipping']) && is_array($data['shipping'])) {
                $transaction->shipping_address = $data['shipping'];
                \Log::info('💾 Shipping salvo na transação ANTES de enviar', [
                    'transaction_id' => $transaction->transaction_id,
                    'shipping' => $data['shipping'],
                ]);
            } else {
                \Log::debug('ℹ️ Nenhum shipping fornecido no $data', [
                    'has_shipping' => isset($data['shipping']),
            ...
            \Log::info('📤 Payload completo sendo enviado para Sharkgateway', [
                'transaction_id' => $transaction->transaction_id,
                'payload' => $payload,
                'has_shipping' => isset($payload['shipping']),
                'shipping_data' => $payload['shipping'] ?? null,
            ]);
    ...
    {
        \Log::info('🔍 prepareSharkgatewayPayload chamado', [
            'transaction_id' => $transaction->transaction_id,
            'has_shipping_in_data' => isset($data['shipping']),
            'has_shipping_in_transaction' => !empty($transaction->shipping_address),
            'data_keys' => array_keys($data),
        ]);
        ...
        // Adicionar telefone do cliente se fornecido
        if (isset($customer['phone']) && !empty($customer['phone'])) {
            $payload['customer']['phone'] = preg_replace('/[^0-9]/', '', $customer['phone']);
        }

        // Adicionar shipping/endereço se fornecido (buscar de $data ou da transação salva)
        $shippingData = $data['shipping'] ?? $transaction->shipping_address ?? null;
        
        \Log::info('🔍 Verificando shipping para Sharkgateway', [
            'transaction_id' => $transaction->transaction_id,
            'has_shipping_in_data' => isset($data['shipping']),
            'has_shipping_in_transaction' => !empty($transaction->shipping_address),
        ...
        if ($shippingData && is_array($shippingData)) {
            // Verificar se tem address dentro do shipping
            if (isset($shippingData['address']) && is_array($shippingData['address'])) {
                $shipping = $shippingData;
                $address = $shipping['address'];
                
                \Log::info('🔍 Endereço encontrado no shipping', [
                    'transaction_id' => $transaction->transaction_id,
                ...
                $requiredFields = ['street', 'streetNumber', 'neighborhood', 'city', 'state', 'zipCode'];
                $missingFields = [];
                ...
                if (empty($missingFields)) {
                    // Converter fee para centavos (pode vir em reais ou já em centavos)
                    $fee = 0;
                    if (isset($shipping['fee'])) {
                        $feeValue = (float)$shipping['fee'];
                        ...
                        $fee = $feeValue < 100 ? (int)round($feeValue * 100) : (int)round($feeValue);
                    }
                    
                    // Montar objeto de endereço conforme documentação PayShark
                    $addressPayload = [
                    ...
                    // Adicionar complement apenas se fornecido (não enviar null)
                    if (isset($address['complement']) && !empty($address['complement'])) {
                        $addressPayload['complement'] = $address['complement'];
                    }
                    ...
                    \Log::info('✅ Shipping adicionado ao payload PayShark (formato correto)', [
                        'transaction_id' => $transaction->transaction_id,
                        'shipping' => $payload['shipping'],
                        'fee_original' => $shipping['fee'] ?? null,
                        'fee_centavos' => $fee,
                    ]);
                } else {
                    \Log::warning('⚠️ Shipping fornecido mas campos obrigatórios faltando', [
                        'transaction_id' => $transaction->transaction_id,
                ...
                \Log::warning('⚠️ Shipping fornecido mas sem campo address', [
                    'transaction_id' => $transaction->transaction_id,
                    'shipping_keys' => array_keys($shippingData),
                    'shipping_data' => $shippingData
                ]);
            }
        } else {
            \Log::debug('ℹ️ Nenhum shipping fornecido para esta transação', [
                'transaction_id' => $transaction->transaction_id,
            ]);
        }
    ...
    private function prepareItems(Transaction $transaction, array $data): array
    {
        ...
        if ($products && is_array($products) && !empty($products)) {
            ...
            foreach ($products as $product) {
                ...
                // Nome do produto (para enviar para adquirente)
                $name = $product['name'] ?? $product['title'] ?? 'Produto';
                
                // Descrição do produto (para enviar para adquirente)
                ...
                // Formato PayShark: apenas campos permitidos pela API
                // title, quantity, tangible, unitPrice são obrigatórios
                // externalRef é opcional
                $item = [
                    'title' => $title,
                    'quantity' => $quantity,
                    'unitPrice' => $unitPrice,
                ...
                // Adicionar externalRef se fornecido (opcional na API PayShark)
                if (isset($product['externalRef']) && !empty($product['externalRef'])) {
                    $item['externalRef'] = $product['externalRef'];
                } elseif (isset($product['external_id']) && !empty($product['external_id'])) {
                    $item['externalRef'] = $product['external_id'];
                }
                
                // Log para debug (name e description não são enviados para PayShark, mas são usados internamente)
                \Log::debug('📦 Item preparado para PayShark', [
                    'title' => $title,
                    'name' => $name, // Não enviado, apenas para log
                    'description' => $description, // Não enviado, apenas para log
                    'quantity' => $quantity,
                    'unitPrice' => $unitPrice,
                    'tangible' => $tangible,
                ]);
        ...
        // Formato PayShark: apenas campos permitidos
        ...
        // Adicionar externalRef se fornecido
        if (isset($data['external_id']) && !empty($data['external_id'])) {
            $defaultItem['externalRef'] = $data['external_id'];
        } elseif (isset($data['externalRef']) && !empty($data['externalRef'])) {
            $defaultItem['externalRef'] = $data['externalRef'];
        }
