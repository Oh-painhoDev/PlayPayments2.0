...
@section('title', 'Detalhes de Faturamento por Período')
@section('page-title', 'Faturamento: ' . $user->name)
@section('page-description', 'Detalhes de faturamento e saques da empresa no período selecionado')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg border border-gray-300 p-6 mb-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('admin.billing-period.index', ['date_from' => $startDate->format('Y-m-d'), 'date_to' => $endDate->format('Y-m-d')]) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm">
                    Voltar
                </a>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg border border-gray-300 p-4 mb-6">
        <form method="GET" action="{{ route('admin.billing-period.show', $user) }}" class="flex flex-wrap gap-4">
            <div class="flex items-center">
                ...
                <input 
                    ...
                    class="bg-gray-100 border border-gray-300 rounded-lg text-gray-900 text-sm px-3 py-2"
                >
            </div>
            ...
            <div class="ml-auto text-right">
                ...
                <p class="text-xs text-gray-600">{{ $endDate->diffInDays($startDate) + 1 }} dias</p>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-300">
            ...
            <p class="text-xs text-gray-600">{{ number_format($totals['total_withdrawals']) }} Saques no Período</p>
        </div>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-300">
            ...
            <p class="text-xs text-gray-600">Saldo Líquido no Período</p>
        </div>
    </div>
    ...
    <div x-data="{ activeTab: 'transactions' }" class="bg-white rounded-lg border border-gray-300 p-6">
        ...
        <div x-show="activeTab === 'transactions'">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Transações Pagas no Período</h3>
            ...
            @if($transactions->count() > 0)
                ...
                @if($transactions->hasPages())
                    <div class="mt-4">
                        {{ $transactions->appends(['date_from' => $startDate->format('Y-m-d'), 'date_to' => $endDate->format('Y-m-d')])->links() }}
                    </div>
                @endif
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600 text-lg">Nenhuma transação encontrada no período selecionado</p>
                </div>
            @endif
        </div>
        ...
        <div x-show="activeTab === 'withdrawals'" x-cloak>
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Saques Concluídos no Período</h3>
            ...
            @if($withdrawals->count() > 0)
                ...
                @if($withdrawals->hasPages())
                    <div class="mt-4">
                        {{ $withdrawals->appends(['date_from' => $startDate->format('Y-m-d'), 'date_to' => $endDate->format('Y-m-d')])->links() }}
                    </div>
                @endif
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600 text-lg">Nenhum saque encontrado no período selecionado</p>
                </div>
            @endif
        </div>
    </div>
