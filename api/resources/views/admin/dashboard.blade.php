...
<div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('admin.dashboard') }}" class="flex flex-wrap gap-4">
            ...
            <div class="ml-auto text-right">
                <p class="text-sm text-gray-600">Período: {{ $startDate->format('d/m/Y') }} até {{ $endDate->format('d/m/Y') }}</p>
                <p class="text-xs text-gray-500">{{ $endDate->diffInDays($startDate) + 1 }} dias</p>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg p-6 border border-gray-200 hover:border-gray-300 transition-all duration-300">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ $stats['total_transactions'] }} transações</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg p-6 border border-gray-200 hover:border-gray-300 transition-all duration-300">
            ...
            <div>
                <p class="text-2xl font-bold text-green-600">R$ {{ number_format($stats['profit_details']['gateway_fees'], 2, ',', '.') }}</p>
                <p class="text-sm text-gray-600 mt-1">Taxas de Gateway</p>
                <p class="text-xs text-gray-500 mt-1">{{ number_format(($stats['profit_details']['gateway_fees'] / max($stats['total_revenue'], 1)) * 100, 2) }}% da receita</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg p-6 border border-gray-200 hover:border-gray-300 transition-all duration-300">
            ...
            <div>
                <p class="text-2xl font-bold text-green-600">R$ {{ number_format($stats['profit_details']['total_profit'], 2, ',', '.') }}</p>
                <p class="text-sm text-gray-600 mt-1">Lucro Líquido</p>
                <p class="text-xs text-gray-500 mt-1">{{ number_format(($stats['profit_details']['total_profit'] / max($stats['total_revenue'], 1)) * 100, 2) }}% de margem</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div class="bg-white rounded-lg p-5 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">R$ {{ number_format($stats['average_ticket'], 2, ',', '.') }}</p>
        ...
        <div class="bg-white rounded-lg p-5 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['paid_transactions'] / max($stats['total_transactions'], 1) * 100, 1) }}%</p>
        ...
        <div class="bg-white rounded-lg p-5 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['refund_info']['total_refund_percentage'], 1) }}%</p>
    ...
    <div class="bg-white rounded-lg p-6 border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-gray-900">R$ {{ number_format($stats['profit_details']['total_user_fees'], 2, ',', '.') }}</p>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-green-600">R$ {{ number_format($stats['profit_details']['gateway_fees'], 2, ',', '.') }}</p>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-orange-400">R$ {{ number_format($stats['refund_info']['total_refund_amount'], 2, ',', '.') }}</p>
                <p class="text-xs text-gray-500 mt-1">{{ $stats['refund_info']['total_refund_count'] }} transações</p>
            </div>
            
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h4 class="text-sm font-medium text-gray-600 mb-2">Lucro por Transação</h4>
                <p class="text-xl font-bold text-gray-900">R$ {{ number_format($stats['profit_details']['transaction_count'] > 0 ? $stats['profit_details']['total_profit'] / $stats['profit_details']['transaction_count'] : 0, 2, ',', '.') }}</p>
    ...
    <div class="bg-white rounded-lg p-6 border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-green-600">R$ {{ number_format($stats['refund_info']['chargeback_amount'], 2, ',', '.') }}</p>
                <p class="text-xs text-gray-500 mt-1">{{ $stats['refund_info']['chargeback_count'] }} transações</p>
            </div>
            
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h4 class="text-sm font-medium text-gray-600 mb-2">Reembolsos Manuais</h4>
                <p class="text-xl font-bold text-orange-400">R$ {{ number_format($stats['refund_info']['manual_refund_amount'], 2, ',', '.') }}</p>
                <p class="text-xs text-gray-500 mt-1">{{ $stats['refund_info']['manual_refund_count'] }} transações</p>
            </div>
            
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h4 class="text-sm font-medium text-gray-600 mb-2">Taxa de Chargeback</h4>
                <p class="text-xl font-bold text-gray-900">{{ number_format($stats['refund_info']['chargeback_percentage'], 2) }}%</p>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-gray-900">{{ number_format($stats['refund_info']['total_refund_percentage'], 2) }}%</p>
                <p class="text-xs text-gray-500 mt-1">Percentual sobre vendas</p>
            </div>
        </div>
    </div>
