...
@section('page-description', 'Gerencie avisos e notificações para os usuários')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden mb-6">
        ...
        <form action="{{ route('admin.white-label.announcements.store') }}" method="POST" class="p-6">
            @csrf
                    ...
                    @forelse($announcements as $announcement)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">{{ Str::limit($announcement->message, 50) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $typeColors = [
                                ...
                                @endphp
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $typeColors[$announcement->type] ?? 'bg-gray-100 text-gray-800' }}">
                                ...
                                @if($announcement->starts_at || $announcement->ends_at)
                                    <div>
                                        @if($announcement->starts_at)
                                            <div>Início: {{ \Carbon\Carbon::parse($announcement->starts_at)->format('d/m/Y H:i') }}</div>
                                        @endif
                                        @if($announcement->ends_at)
                                            <div>Fim: {{ \Carbon\Carbon::parse($announcement->ends_at)->format('d/m/Y H:i') }}</div>
                                        @endif
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form action="{{ route('admin.white-label.announcements.destroy', $announcement->id) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir este aviso?');">
                                    @csrf
                                    @method('DELETE')
