...
@section('page-title', 'Gerenciar UTMify')
@section('page-description', 'Gerencie as integrações UTMify dos usuários')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden mb-6">
        ...
        <form action="{{ route('admin.white-label.utmify.store') }}" method="POST" class="p-6">
            @csrf
                    ...
                    @forelse($integrations as $integration)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $integration->user?->name ?? 'N/A' }}</div>
                                        <div class="text-xs text-gray-500">{{ $integration->user?->email ?? 'N/A' }}</div>
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $integration->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-xs font-mono text-gray-600">{{ substr($integration->api_token, 0, 20) }}...</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    @if($integration->trigger_on_creation)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                            Criação
                                        </span>
                                    @endif
                                    @if($integration->trigger_on_payment)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button 
                                    onclick="openEditModal({{ $integration->id }}, '{{ addslashes($integration->name) }}', '{{ $integration->api_token }}', {{ $integration->trigger_on_creation ? 'true' : 'false' }}, {{ $integration->trigger_on_payment ? 'true' : 'false' }}, {{ $integration->is_active ? 'true' : 'false' }}, {{ $integration->isGlobal() ? 'true' : 'false' }}, {{ $integration->user_id ?? 'null' }})"
                                    class="text-green-600 hover:text-green-900 mr-3"
                                >
                                    Editar
                                </button>
                                <form action="{{ route('admin.white-label.utmify.destroy', $integration->id) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta integração?');">
                                    @csrf
                                    @method('DELETE')
...
function openEditModal(id, name, apiToken, triggerOnCreation, triggerOnPayment, isActive, isGlobal, userId) {
    document.getElementById('editForm').action = '{{ route("admin.white-label.utmify.update", ":id") }}'.replace(':id', id);
