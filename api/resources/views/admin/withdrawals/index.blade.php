...
@section('page-title', 'Gerenciar Saques')
@section('page-description', 'Visualize e aprove solicitações de saque dos usuários')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            <div class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('admin.withdrawals.index', ['status' => 'pending']) }}" class="text-xs text-yellow-500 hover:text-yellow-400">Ver todos</a>
            </div>
            <p class="text-lg font-bold text-gray-900">{{ number_format($pendingCount) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            <div class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('admin.withdrawals.index', ['status' => 'processing']) }}" class="text-xs text-blue-500 hover:text-blue-600">Ver todos</a>
            </div>
            <p class="text-lg font-bold text-gray-900">{{ number_format($processingCount) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            <div class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('admin.withdrawals.index', ['status' => 'completed']) }}" class="text-xs text-green-500 hover:text-green-600">Ver todos</a>
            </div>
            <p class="text-lg font-bold text-gray-900">{{ number_format($completedCount) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            <div class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('admin.withdrawals.index', ['status' => 'failed']) }}" class="text-xs text-green-500 hover:text-green-600">Ver todos</a>
            </div>
            <p class="text-lg font-bold text-gray-900">{{ number_format($failedCount) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">R$ {{ number_format($totalAmount, 2, ',', '.') }}</p>
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($withdrawals as $withdrawal)
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-gray-900 font-medium">{{ $withdrawal->user->name }}</p>
                                    <p class="text-gray-600 text-sm">{{ $withdrawal->user->email }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-gray-900 font-medium">{{ $withdrawal->formatted_amount }}</p>
                                    <p class="text-gray-600 text-sm">Taxa: {{ $withdrawal->formatted_fee }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-gray-900 text-sm">{{ $withdrawal->pix_type_label }}</p>
                                    <p class="text-gray-600 text-xs">{{ Str::mask($withdrawal->pix_key, '*', 4) }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <span class="px-3 py-1 text-xs font-medium rounded-full border {{ $config['class'] }}">
                                    {{ $config['label'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.withdrawals.show', $withdrawal) }}" class="text-blue-600 hover:text-blue-700 text-sm">
                                        Ver
                                    </a>
                                    @if($withdrawal->status === 'pending' && $withdrawal->user->withdrawal_type === 'manual')
                                        <button 
                                            onclick="openApprovalModal({{ $withdrawal->id }})"
