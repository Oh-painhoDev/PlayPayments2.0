                        ...
                        @if($transaction->payment_method === 'pix')
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="text-xs font-semibold text-white">{{ strlen($pixPayload) > 20 ? substr($pixPayload, 0, 20) . '...' : $pixPayload }}</span>
                            ...
                            <div id="qrCodeContainer" class="hidden flex justify-center mt-4">
                                <img alt="QR Code PIX" loading="lazy" width="200" height="200" decoding="async" class="rounded-lg bg-white p-2" src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={{ urlencode($pixPayload) }}">
                            </div>
                            @endif
                        @endif
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-xs font-semibold text-[#21b3dd]">-R$ {{ number_format($transaction->fee_amount, 2, ',', '.') }}</span>
                        </div>
                        <div class="h-px w-full opacity-20 bg-[#707070]"></div>
                        <div class="flex justify-between items-center">
        ...
        <div class="w-full">
            <div class="flex flex-col gap-4">
                ...
                <div class="w-full h-[7px] rounded-[40px] bg-[#1f1f1f]">
                    <div class="h-full bg-[#21b3dd] rounded-[40px] transition-all duration-500" style="width: {{ $deliveryProgress }}%;"></div>
                </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="text-xs font-semibold text-white">
                            @if(isset($transaction->customer_data['address']) && is_array($transaction->customer_data['address']))
                                <p class="leading-[1.3]">{{ $transaction->customer_data['address']['street'] ?? 'N/A' }}, {{ $transaction->customer_data['address']['number'] ?? '' }}</p>
                                <p class="leading-[1.3]">{{ $transaction->customer_data['address']['neighborhood'] ?? '' }}, {{ $transaction->customer_data['address']['city'] ?? '' }} - {{ $transaction->customer_data['address']['state'] ?? '' }}</p>
                                <p class="leading-[1.3]">{{ $transaction->customer_data['address']['zipcode'] ?? '' }}</p>
                            @else
