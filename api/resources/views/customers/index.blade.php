    ...
    <div class="container mx-auto px-4 py-6 md:px-6 lg:px-8">
        ...
        <div class="hidden md:flex items-center justify-between mb-6">
            <div class="flex flex-col gap-2.5 items-start justify-start">
                ...
                <p class="font-['Manrope'] font-semibold text-[12px] tracking-[-0.24px] text-[#707070]">Total de {{ $totalCustomers }} clientes cadastrados</p>
            </div>
            <div class="flex gap-2 items-center">
                ...
                <button onclick="openAddCustomerModal()" class="bg-[#21b3dd] flex gap-2 items-center justify-center px-4 py-2.5 rounded-lg cursor-pointer hover:opacity-90 transition-opacity">
        ...
        <div id="filtersPanel" class="hidden mb-6 p-4 bg-[#161616] rounded-lg">
            <form method="GET" action="{{ route('customers.index') }}" class="flex flex-col md:flex-row gap-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por nome, CPF, email ou telefone..." class="flex-1 px-4 py-2 rounded-md bg-[#1F1F1F] text-white border-0 h-[42px] text-[12px] font-semibold font-['Manrope'] tracking-[-0.24px] placeholder:text-[#707070]">
                <button type="submit" class="bg-[#21b3dd] text-white px-4 py-2 rounded-md hover:opacity-90 transition-opacity">Buscar</button>
                @if(request('search'))
                    <a href="{{ route('customers.index') }}" class="bg-[#1F1F1F] text-[#707070] px-4 py-2 rounded-md hover:opacity-80 transition-opacity">Limpar</a>
                @endif
            </form>
        </div>
                    ...
                    <div class="bg-[#1F1F1F] rounded-lg p-4 cursor-pointer transition-all hover:opacity-80">
                        ...
                        <div class="md:hidden">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[12px] font-semibold tracking-[-0.24px] text-white">{{ $customer['name'] }}</span>
                                </div>
                        ...
                        <div class="hidden md:flex items-center gap-6">
                            <div class="flex items-center gap-2 w-[300px]">
                                <div class="bg-[#161616] w-8 h-8 rounded-lg flex items-center justify-center">
                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M9 9C11.0711 9 12.75 7.32107 12.75 5.25C12.75 3.17893 11.0711 1.5 9 1.5C6.92893 1.5 5.25 3.17893 5.25 5.25C5.25 7.32107 6.92893 9 9 9Z" stroke="#21b3dd" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                        <path d="M15.75 16.5C15.75 13.6025 12.6975 11.25 9 11.25C5.3025 11.25 2.25 13.6025 2.25 16.5" stroke="#21b3dd" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                    </svg>
                                </div>
                                <span class="text-[12px] font-semibold tracking-[-0.24px] text-white">{{ $customer['name'] }}</span>
                            </div>
                            ...
                            <div class="w-[200px]">
                                <span class="text-[12px] font-semibold tracking-[-0.24px] text-[#707070]">{{ $customer['phone'] ?? 'N/A' }}</span>
                            </div>
                            <div class="flex-1">
                                <span class="text-[12px] font-semibold tracking-[-0.24px] text-[#707070]">
                                    @if($customer['address'])
            ...
            @if($customers->hasPages())
                <div class="flex items-center justify-center w-full mt-6">
                    <div class="flex items-center gap-2">
                        ...
                        @else
                            <a href="{{ $customers->previousPageUrl() }}" class="h-8 px-3 py-1 rounded-md flex items-center gap-1.5 bg-[#1f1f1f] hover:bg-[#2a2a2a] text-[#aaaaaa] hover:text-white transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-left h-3 w-3">
                                    <path d="m15 18-6-6 6-6"></path>
                                </svg>
                                <span class="text-[12px] font-semibold font-['Manrope'] tracking-[-0.24px]">Anterior</span>
                        ...
                        @if($customers->hasMorePages())
                            <a href="{{ $customers->nextPageUrl() }}" class="h-8 px-3 py-1 rounded-md flex items-center gap-1.5 bg-[#1f1f1f] hover:bg-[#2a2a2a] text-[#aaaaaa] hover:text-white transition-colors">
...
<div id="addCustomerModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden" style="background-color: rgba(0, 0, 0, 0.5);">
    <div class="relative w-full max-w-lg rounded-2xl p-[30px] bg-[#161616] border-0">
        <div class="flex justify-between items-start mb-4">
            ...
            <button onclick="closeAddCustomerModal()" class="text-[#707070] hover:text-[#21b3dd] transition-colors">
        ...
        <form id="addCustomerForm" onsubmit="submitAddCustomer(event)" class="space-y-4">
            ...
            <div role="radiogroup" class="gap-2 flex space-x-4 mb-4">
                <div class="flex items-center space-x-2">
                    <input type="radio" id="individual" name="type" value="individual" checked class="aspect-square h-4 w-4 rounded-full border-2 border-[#21b3dd] text-[#21b3dd] bg-[#161616] focus:ring-2 focus:ring-[#21b3dd] focus:ring-offset-2">
                    <label for="individual" class="text-[12px] font-semibold tracking-[-0.24px] text-white cursor-pointer">Pessoa Física</label>
                </div>
                <div class="flex items-center space-x-2">
                    <input type="radio" id="business" name="type" value="business" class="aspect-square h-4 w-4 rounded-full border-2 border-[#21b3dd] text-[#21b3dd] bg-[#161616] focus:ring-2 focus:ring-[#21b3dd] focus:ring-offset-2">
            ...
            <!-- Botão Submit -->
            <button type="submit" class="inline-flex items-center justify-center gap-2 w-full h-[38px] bg-[#21b3dd] text-white text-[14px] font-semibold tracking-[-0.28px] rounded-lg hover:bg-[#21b3dd]/90 transition-all">Confirmar e continuar</button>
        </form>
    </div>
</div>
