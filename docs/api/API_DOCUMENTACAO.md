...
## 2.1. Rotas Rápidas (Resumo MazorPay)

| Método | Rota | Descrição |
| --- | --- | --- |
| `GET` | `/v1/transactions` | Lista transações do tenant autenticado |
| `POST` | `/v1/transactions` | Cria nova venda (PIX, boleto ou cartão) |
| `GET` | `/v1/transactions/{id}` | Detalha transação específica |
| `POST` | `/v1/transactions/{id}/refund` | Solicita estorno/chargeback |
| `GET` | `/v1/customers` | Lista clientes extraídos das vendas |
| `POST` | `/v1/customers` | Cadastra cliente manualmente |
| `GET` | `/v1/balance` | Consulta saldo disponível/retido |
| `POST` | `/v1/withdrawals` | Solicita saque Pix-out |
| `GET` | `/webhooks` | Lista webhooks cadastrados |
| `POST` | `/webhooks/test` | Dispara evento de teste para URL configurada |

> Todas as rotas seguem o mesmo padrão de autenticação via `X-Public-Key` e `X-Private-Key`. Para brincar localmente basta apontar o base-url para `http://localhost:8000`.
