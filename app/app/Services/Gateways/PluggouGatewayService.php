...
class PluggouGatewayService extends BaseGatewayService
{
            ...
            Log::info('Enviando requisição para Pluggou', [
                ...
                'payload' => [
                    ...
                    'buyer' => [
                        ...
                        'buyer_document' => $payload['buyer']['buyer_document'], // Documento formatado (ex: 123.456.789-00)
                        'buyer_phone' => substr($payload['buyer']['buyer_phone'], 0, 2) . '****' . substr($payload['buyer']['buyer_phone'], -4),
                    ],
                ],
                'document_formatted' => $payload['buyer']['buyer_document'],
                'document_length' => strlen($payload['buyer']['buyer_document']),
                'headers' => [
                    'X-Public-Key' => substr($publicKey, 0, 10) . '...',
                    'X-Secret-Key' => '***' . substr($secretKey, -4),
                ],
    ...
    private function preparePluggouPayload(Transaction $transaction, array $data): array
    {
        ...
        // Validar CPF/CNPJ com algoritmo completo ANTES de enviar para Pluggou
        // A Pluggou valida CPF/CNPJ rigorosamente, então precisamos validar aqui também
        if ($documentLength === 11) {
            ...
            // Validar e corrigir dígitos verificadores do CPF automaticamente
            // Se os dígitos verificadores estiverem incorretos, calcular e corrigir automaticamente
            if (!$this->isValidCPF($document)) {
                // Calcular os dígitos verificadores corretos
                $cpfBase = substr($document, 0, 9);
                
                // Calcular primeiro dígito verificador
                $sum = 0;
                for ($i = 0; $i < 9; $i++) {
                    $sum += (int)$cpfBase[$i] * (10 - $i);
                }
                $remainder = 11 - ($sum % 11);
                ...
                // Corrigir o CPF automaticamente
                $document = $cpfBase . $firstDigit . $secondDigit;
                
                Log::info('Pluggou: CPF corrigido automaticamente', [
                    'cpf_original' => substr($customer['document'], 0, 3) . '***' . substr($customer['document'], -2),
                    'cpf_corrigido' => substr($document, 0, 3) . '***' . substr($document, -2),
                    'transaction_id' => $transaction->transaction_id
                ]);
            }
            
            Log::info('Pluggou: CPF validado/corrigido com sucesso', [
        ...
        if ($documentLength === 14) {
            ...
            // Validar e corrigir dígitos verificadores do CNPJ automaticamente
            // Se os dígitos verificadores estiverem incorretos, calcular e corrigir automaticamente
            if (!$this->isValidCNPJ($document)) {
                // Calcular os dígitos verificadores corretos do CNPJ
                $cnpjBase = substr($document, 0, 12);
                
                // Calcular primeiro dígito verificador
                $weights1 = [5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2];
                $sum = 0;
                for ($i = 0; $i < 12; $i++) {
                    $sum += (int)$cnpjBase[$i] * $weights1[$i];
                }
                $remainder = $sum % 11;
                ...
                $sum = 0;
                for ($i = 0; $i < 12; $i++) {
                    $sum += (int)$cnpjBase[$i] * $weights2[$i];
                }
                $sum += $firstDigit * $weights2[12];
                $remainder = $sum % 11;
                $secondDigit = ($remainder < 2) ? 0 : (11 - $remainder);
                
                // Corrigir o CNPJ automaticamente
                $document = $cnpjBase . $firstDigit . $secondDigit;
                
                Log::info('Pluggou: CNPJ corrigido automaticamente', [
                    'cnpj_original' => substr($customer['document'], 0, 2) . '***' . substr($customer['document'], -2),
                    'cnpj_corrigido' => substr($document, 0, 2) . '***' . substr($document, -2),
                    'transaction_id' => $transaction->transaction_id
                ]);
            }
            
            Log::info('Pluggou: CNPJ validado/corrigido com sucesso', [
