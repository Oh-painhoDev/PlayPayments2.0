...
class Transaction extends Model
{
    ...
    protected static function boot()
    {
        ...
        // Create referral commission when transaction is paid
        static::updated(function ($transaction) {
            // Check if status changed to 'paid' and paid_at was set
            if ($transaction->status === 'paid' && $transaction->paid_at && $transaction->wasChanged('status')) {
                static::createReferralCommission($transaction);
            }
        });
    }
    
    /**
     * Create referral commission for this transaction
     */
    protected static function createReferralCommission(Transaction $transaction)
    {
        try {
            ...
            // Get commission percentage and fixed fee from referred user
            $commissionPercentage = $user->commission_percentage ?? 1.00;
            $commissionFixed = $user->commission_fixed ?? 0.00;
            
            // Calculate commission amount
            $commissionAmount = ($transaction->amount * $commissionPercentage / 100) + $commissionFixed;
            ...
            $existingCommission = \App\Models\ReferralCommission::where('transaction_id', $transaction->id)
            ...
            \Illuminate\Support\Facades\Log::info('Comissão de referência criada', [
                'referrer_id' => $referrer->id,
                'referred_id' => $user->id,
                'transaction_id' => $transaction->id,
                'commission_amount' => $commissionAmount,
            ...
            \Illuminate\Support\Facades\Log::error('Erro ao criar comissão de referência', [
                'transaction_id' => $transaction->id,
                'error' => $e->getMessage(),
