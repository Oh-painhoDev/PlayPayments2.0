...
class CustomerController extends Controller
{
        ...
        foreach ($transactions as $transaction) {
            ...
            if (isset($customersMap[$document])) {
                ...
                if (empty($customer['address'])) {
                    $customer['address'] = $this->formatAddress($this->extractCustomerAddress($customerData));
                }
            } else {
                // Criar novo cliente
                $customersMap[$document] = [
                    ...
                    'address' => $this->formatAddress($this->extractCustomerAddress($customerData)),
    ...
    private function formatAddress($address)
    {
        ...
        if (!empty($cityLine)) {
            $segments[] = implode(', ', $cityLine);
        }
        
        $zip = $address['zipcode'] ?? $address['zip_code'] ?? $address['zipCode'] ?? $address['postalCode'] ?? null;
        if ($zip) {
            $segments[] = $zip;
        }
    ...
    /**
     * Extrai endereço mesmo quando vem em campos soltos
     */
    private function extractCustomerAddress(array $customerData): ?array
    {
        ...
        $street = $customerData['street'] ?? $customerData['street_name'] ?? $customerData['streetName'] ?? null;
        if ($street) {
            $address['street'] = $street;
        }
        
        $number = $customerData['number'] ?? $customerData['streetNumber'] ?? $customerData['street_number'] ?? null;
        if ($number) {
            $address['number'] = $number;
        }
        
        if (!empty($customerData['complement'])) {
            $address['complement'] = $customerData['complement'];
        }
        
        $neighborhood = $customerData['neighborhood'] ?? $customerData['district'] ?? null;
        if ($neighborhood) {
        ...
        if (!empty($customerData['state'])) {
            $address['state'] = strtoupper($customerData['state']);
        }
        
        $zip = $customerData['zipcode'] ?? $customerData['zip_code'] ?? $customerData['zipCode'] ?? $customerData['postalCode'] ?? $customerData['cep'] ?? null;
        if ($zip) {
            $address['zipcode'] = preg_replace('/[^0-9]/', '', $zip) ?: $zip;
        }
