...
class WebhookController extends Controller
{
    ...
    public function pluggou(Request $request)
    {
        try {
            ...
            // Processar webhooks de transação e saque
            if ($data['event_type'] === 'withdrawal') {
                return $this->handlePluggouWithdrawalWebhook($data, $request);
            }
            
            if ($data['event_type'] !== 'transaction') {
                Log::info('Webhook Pluggou processado - tipo de evento não é transação nem saque', [
                    'event_type' => $data['event_type'],
    ...
    /**
     * Handle PluggouCash withdrawal webhook
     * 
     * Estrutura do webhook de saque:
     * {
     *   "id": "e955fa54-0e48-47fc-9439-652d88063100",
     *   "event_type": "withdrawal",
     *   "data": {
     *     "id": "7c87236c-bec1-4bff-8832-8e8b86b7e8c1",
     *     "e2e_id": "ES882I1IS9DUA8SDY81U2IJ3",
     *     "amount": 2000,
     *     "liquid_amount": 1850,
     *     "pix_type": "cpf",
     *     "pix_key": "000.000.000-00",
     *     "status": "paid",
     *     "paid_at": "2025-03-25 10:12:48",
     *     "created_at": "2025-10-20 04:33:31"
     *   }
     * }
     */
    protected function handlePluggouWithdrawalWebhook(array $data, Request $request)
    {
        try {
            $withdrawalData = $data['data'];
            $webhookId = $data['id'] ?? null;
            $externalId = $withdrawalData['id'] ?? null;
            $status = $withdrawalData['status'] ?? 'pending';

            Log::info('Processando webhook de saque PluggouCash', [
                'webhook_id' => $webhookId,
                'external_id' => $externalId,
                'status' => $status,
            ...
            if (!$externalId) {
                Log::warning('Webhook PluggouCash de saque inválido - ID ausente', $data);
                return response()->json([
                    'success' => false,
                    'error' => 'ID do saque ausente'
                ], 400);
            }

            // Buscar saque pelo external_id (ID do PluggouCash)
            $withdrawal = \App\Models\Withdrawal::where('external_id', $externalId)->first();

            // Se não encontrar pelo external_id, tentar buscar pelo e2e_id no response_data
            if (!$withdrawal && isset($withdrawalData['e2e_id'])) {
                $e2eId = $withdrawalData['e2e_id'];
                Log::info('Tentando buscar saque pelo e2e_id', [
                    'e2e_id' => $e2eId,
                    'external_id' => $externalId
                ]);
                
                // Buscar em todos os saques recentes e verificar response_data
                $withdrawals = \App\Models\Withdrawal::where(function($q) {
                ...
                foreach ($withdrawals as $w) {
                    $responseData = $w->response_data ?? [];
                    if (isset($responseData['e2e_id']) && $responseData['e2e_id'] === $e2eId) {
                        $withdrawal = $w;
                        Log::info('Saque encontrado pelo e2e_id no response_data', [
                            'withdrawal_id' => $w->withdrawal_id,
                            'e2e_id' => $e2eId
                        ]);
                        // Atualizar external_id se não estiver salvo
                        if (!$w->external_id && $externalId) {
                            $w->update(['external_id' => $externalId]);
                        }
                        break;
                    }
                }
            ...
            // Se ainda não encontrou, tentar buscar pelo ID no response_data
            if (!$withdrawal) {
                Log::info('Tentando buscar saque pelo ID no response_data', [
                    'external_id' => $externalId
                ]);
                ...
                foreach ($withdrawals as $w) {
                    $responseData = $w->response_data ?? [];
                    if (isset($responseData['id']) && $responseData['id'] === $externalId) {
                        $withdrawal = $w;
                        Log::info('Saque encontrado pelo ID no response_data', [
                            'withdrawal_id' => $w->withdrawal_id,
                            'external_id' => $externalId
                        ]);
                        // Atualizar external_id no banco
                        $w->update(['external_id' => $externalId]);
                        break;
                    }
                }
            ...
            // Última tentativa: buscar por pix_key e amount (se disponível)
            if (!$withdrawal && isset($withdrawalData['pix_key']) && isset($withdrawalData['amount'])) {
                $pixKey = $withdrawalData['pix_key'];
                $amount = $withdrawalData['amount'] / 100; // Converter centavos para reais
                
                Log::info('Tentando buscar saque por pix_key e amount', [
                    'pix_key' => $pixKey,
                    'amount' => $amount,
                    'external_id' => $externalId
                ]);
                
                $withdrawal = \App\Models\Withdrawal::where('pix_key', $pixKey)
                    ->where('amount', $amount)
                    ->where(function($q) {
                        $q->where('status', 'processing')
                          ->orWhere('status', 'pending');
                ...
                if ($withdrawal) {
                    Log::info('Saque encontrado por pix_key e amount', [
                        'withdrawal_id' => $withdrawal->withdrawal_id,
                        'external_id' => $externalId
                    ]);
                    // Atualizar external_id no banco
                    $withdrawal->update(['external_id' => $externalId]);
                }
            }

            if (!$withdrawal) {
                Log::warning('Saque não encontrado para webhook PluggouCash', [
                    'external_id' => $externalId,
                    'webhook_id' => $webhookId,
                    'e2e_id' => $withdrawalData['e2e_id'] ?? null,
                    'pix_key' => $withdrawalData['pix_key'] ?? null,
                    'amount' => $withdrawalData['amount'] ?? null,
                    'recent_withdrawals' => \App\Models\Withdrawal::where('created_at', '>=', now()->subDays(7))
                        ->select('withdrawal_id', 'external_id', 'status', 'amount', 'pix_key')
                        ->get()
                        ->toArray()
                ]);
                return response()->json([
            ...
            // Mapear status do PluggouCash para nosso status
            $newStatus = $this->mapPluggouWithdrawalStatus($status);
            $oldStatus = $withdrawal->status;

            Log::info('Status de saque PluggouCash mapeado', [
                'original_status' => $status,
                'mapped_status' => $newStatus,
                'old_status' => $oldStatus,
                'withdrawal_id' => $withdrawal->withdrawal_id
            ...
            // Atualizar external_id se necessário
            if ($externalId && $externalId !== $withdrawal->external_id) {
                $updateData['external_id'] = $externalId;
            }

            // Atualizar response_data com dados do webhook
            $responseData = $withdrawal->response_data ?? [];
            $responseData['webhook'] = $withdrawalData;
            $responseData['webhook_id'] = $webhookId;
            $responseData['last_webhook_received_at'] = now()->toDateTimeString();
            $updateData['response_data'] = $responseData;

            // Definir completed_at se status mudou para completed
            if ($newStatus === 'completed' && $oldStatus !== 'completed') {
                $updateData['completed_at'] = isset($withdrawalData['paid_at']) 
                    ? \Carbon\Carbon::parse($withdrawalData['paid_at'])
                    : now();
            }
            ...
            Log::info('Webhook de saque PluggouCash processado com sucesso', [
                'withdrawal_id' => $withdrawal->withdrawal_id,
                'external_id' => $externalId,
                'old_status' => $oldStatus,
                'new_status' => $newStatus,
            ...
            Log::error('Erro ao processar webhook de saque PluggouCash: ' . $e->getMessage(), [
                'data' => $data,
                'trace' => $e->getTraceAsString()
            ]);
            return response()->json([
    ...
    /**
     * Map PluggouCash withdrawal status to our status
     */
    protected function mapPluggouWithdrawalStatus(string $status): string
    {
        return match(strtolower($status)) {
     ...
     */
    protected function handleWithdrawalStatusChange(\App\Models\Withdrawal $withdrawal, string $oldStatus, string $newStatus)
    {
        // Aqui você pode adicionar lógica adicional quando o status do saque muda
        // Por exemplo, notificar o usuário, atualizar saldo, etc.
        Log::info('Status de saque alterado', [
            'withdrawal_id' => $withdrawal->withdrawal_id,
            'old_status' => $oldStatus,
            'new_status' => $newStatus
        ]);
