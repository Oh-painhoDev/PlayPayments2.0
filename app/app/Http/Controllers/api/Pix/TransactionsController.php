...
class TransactionsController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Formatar e retornar resposta completa
    ...
    /**
     * Formatar transação para resposta da API
     * Retorna JSON completo com todos os dados necessários
     * 
    ...
    private function formatTransaction(Transaction $transaction): array
    {
        ...
        if ($transaction->payment_method === 'pix' && $transaction->payment_data) {
            // Tentar múltiplos formatos de estrutura de dados PIX
            $paymentData = $transaction->payment_data;
            
            // Log para debug
            \Log::info('formatTransaction: Extraindo dados PIX', [
                'transaction_id' => $transaction->transaction_id,
                'gateway_name' => $transaction->gateway->name ?? 'unknown',
                'payment_data_type' => gettype($paymentData),
                'payment_data_keys' => is_array($paymentData) ? array_keys($paymentData) : 'not_array',
                'has_payment_data_pix' => isset($paymentData['payment_data']['pix']),
                'has_pix' => isset($paymentData['pix']),
                'has_data_pix' => isset($paymentData['data']['pix']),
            ]);
            ...
            if ($pixData) {
                \Log::info('formatTransaction: Dados PIX encontrados', [
                    'transaction_id' => $transaction->transaction_id,
                    'pix_data_type' => gettype($pixData),
                    'pix_data_keys' => is_array($pixData) ? array_keys($pixData) : 'not_array',
                    'has_payload' => isset($pixData['payload']),
                    'has_qrcode' => isset($pixData['qrcode']),
                    'has_emv' => isset($pixData['emv']),
                    'has_qr_code' => isset($pixData['qr_code']),
                ]);
                
                // Extrair código PIX (pode estar em vários campos)
                ...
                if (empty($pixCode)) {
                    \Log::error('formatTransaction: Código PIX vazio após extração', [
                        'transaction_id' => $transaction->transaction_id,
                        'pix_data' => $pixData,
                    ]);
                } else {
                    \Log::info('formatTransaction: Código PIX extraído com sucesso', [
                        'transaction_id' => $transaction->transaction_id,
                        'pix_code_length' => strlen($pixCode),
                        'pix_code_preview' => substr($pixCode, 0, 50) . '...',
                    ]);
                }
        ...
        // Carregar gateway se não estiver carregado
        ...
        if ($transaction->payment_method === 'pix') {
            $response['pix'] = [
                ...
                'qrcode' => $pixQrCode ?? $pixCode, // Alias para compatibilidade
