...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // Calculate withdrawal fees earned and total withdrawn
        $withdrawalStats = $this->calculateWithdrawalStats();
        $stats['withdrawal_stats'] = $withdrawalStats;
        
        // Get chart data for sales
        $chartData = $this->getSalesChartData($startDate, $endDate);
        
        // Get sales by location/country
        $salesByLocation = $this->getSalesLocationsByCountry($startDate, $endDate);
    ...
    public function getUserFees(User $user)
    {
        try {
            ...
            $withdrawalFees = [
                'fee_type' => $user->withdrawal_fee_type ?? 'global',
    ...
    public function saveUserFees(Request $request, User $user)
    {
        try {
            ...
            // Add conditional validation for withdrawal fees
            if ($request->withdrawal_fee_type === 'fixed' || $request->withdrawal_fee_type === 'both') {
                $rules['withdrawal_fixed_fee'] = 'required|numeric|min:0';
            }
            
            if ($request->withdrawal_fee_type === 'percentage' || $request->withdrawal_fee_type === 'both') {
                $rules['withdrawal_percentage_fee'] = 'required|numeric|min:0|max:100';
            }
            
            $request->validate($rules, [
                'withdrawal_fixed_fee.required' => 'A taxa fixa é obrigatória quando o tipo é fixo ou ambos',
                'withdrawal_percentage_fee.required' => 'A taxa percentual é obrigatória quando o tipo é percentual ou ambos',
            ]);
            ...
            // Note: Columns don't allow NULL, so we use 0.00 as default
            if ($request->withdrawal_fee_type === 'global') {
                // When global, set to 0.00 - the system will use global fees from FeeHelper
                $updateData['withdrawal_fee_fixed'] = 0.00;
                $updateData['withdrawal_fee_percentage'] = 0.00;
            } elseif ($request->withdrawal_fee_type === 'fixed') {
                $updateData['withdrawal_fee_fixed'] = $request->withdrawal_fixed_fee ?? 0.00;
                $updateData['withdrawal_fee_percentage'] = 0.00;
            } elseif ($request->withdrawal_fee_type === 'percentage') {
                $updateData['withdrawal_fee_fixed'] = 0.00;
                $updateData['withdrawal_fee_percentage'] = $request->withdrawal_percentage_fee ?? 0.00;
            } elseif ($request->withdrawal_fee_type === 'both') {
                $updateData['withdrawal_fee_fixed'] = $request->withdrawal_fixed_fee ?? 0.00;
                $updateData['withdrawal_fee_percentage'] = $request->withdrawal_percentage_fee ?? 0.00;
            }
    ...
    /**
     * Get sales aggregated by location/country
     */
    private function getSalesLocationsByCountry($startDate, $endDate)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            // 3. Tentar obter país de shipping_address ou customer_data
            if (!empty($shippingAddress['country'])) {
                $country = $shippingAddress['country'];
                $city = $shippingAddress['city'] ?? $city;
            } elseif (!empty($customerData['country'])) {
                $country = $customerData['country'];
                $city = $customerData['city'] ?? $city;
            } else {
                // 4. Tentar extrair de metadata/IP
                $metadata = $transaction->metadata;
                if (is_string($metadata)) {
                    $metadata = json_decode($metadata, true);
                }
                // IP location seria necessário chamar API externa
            }
        ...
        return array_slice($locationCounts, 0, 10);
    }
    ...
    /**
     * Calculate withdrawal statistics (fees earned and total withdrawn)
     */
    private function calculateWithdrawalStats()
    {
        // Buscar ou criar wallet do admin
        $admin = auth()->user();
        ...
        // IMPORTANTE: Sincronizar taxas dos saques já aprovados que ainda não foram creditadas
        $this->syncAdminFees($adminWallet);
        
        // Recarregar wallet para pegar o saldo atualizado
        $adminWallet->refresh();
        
        // Total de taxas ganhas (soma de todas as taxas de saques aprovados/completados)
        // Essas são as taxas que o admin ganha quando os usuários fazem saques
        $totalFeesEarned = Withdrawal::whereIn('status', ['completed', 'approved'])
            ->sum('fee');
        
        // Total pago aos usuários (soma de todos os valores de saques aprovados/completados)
        // Isso é o valor que foi pago aos usuários (amount, não inclui a taxa)
        $totalPaidToUsers = Withdrawal::whereIn('status', ['completed', 'approved'])
            ->sum('amount');
        
        // Total debitado das carteiras dos usuários (amount + fee)
        // Isso é o que foi debitado das carteiras dos usuários
        $totalDebitedFromUsers = Withdrawal::whereIn('status', ['completed', 'approved'])
            ->selectRaw('SUM(amount + fee) as total')
            ->value('total') ?? 0;
        
        // Saldo disponível = Saldo da wallet do admin (que recebe as taxas)
        $availableBalance = $adminWallet->available_balance ?? 0;
        
        // Total de saques aprovados
        $totalApprovedWithdrawals = Withdrawal::whereIn('status', ['completed', 'approved'])
            ->count();
        ...
        return [
            ...
            'total_withdrawn' => $totalPaidToUsers, // Total pago aos usuários
            'total_debited_from_users' => $totalDebitedFromUsers, // Total debitado dos usuários
            'available_balance' => $availableBalance, // Saldo disponível da wallet do admin
    ...
    /**
     * Sincronizar taxas dos saques aprovados que ainda não foram creditadas na wallet do admin
     */
    private function syncAdminFees($adminWallet)
    {
        // Buscar todos os saques aprovados/completados
        $approvedWithdrawals = Withdrawal::whereIn('status', ['completed', 'approved'])
            ->where('fee', '>', 0)
            ->get();
        ...
        foreach ($approvedWithdrawals as $withdrawal) {
            // Verificar se já existe uma transação de crédito para este saque
            $existingCredit = \App\Models\WalletTransaction::where('wallet_id', $adminWallet->id)
                ->where('reference_id', $withdrawal->withdrawal_id . '_admin_fee')
                ->where('type', 'credit')
                ->where('category', 'withdrawal_fee')
                ->first();
            
            // Se não existe, creditar a taxa
            if (!$existingCredit && $withdrawal->fee > 0) {
                try {
                    $adminWallet->addCredit(
                        $withdrawal->fee,
                        'withdrawal_fee',
                        "Taxa de saque aprovado (sincronizado) - {$withdrawal->withdrawal_id}",
                        [
                    ...
                    $creditedCount++;
                    $totalCredited += $withdrawal->fee;
                } catch (\Exception $e) {
                    Log::error('Erro ao sincronizar taxa do admin', [
                        'withdrawal_id' => $withdrawal->withdrawal_id,
                        'fee' => $withdrawal->fee,
                        'error' => $e->getMessage()
                    ]);
        ...
        // Se creditou alguma taxa, logar
        if ($creditedCount > 0) {
            Log::info('Taxas do admin sincronizadas', [
                'admin_id' => auth()->id(),
    ...
    /**
     * Adjust admin wallet balance manually
     */
    public function adjustWalletBalance(Request $request)
    {
        try {
            ...
            $description = $request->description ?? ($request->type === 'add' ? 'Ajuste manual - Adição' : 'Ajuste manual - Remoção');
            ...
            if ($request->type === 'add') {
                ...
                $adminWallet->addCredit(
                    ...
                    'admin_adjustment_' . time()
                );
                
                $message = "R$ " . number_format($amount, 2, ',', '.') . " adicionado com sucesso!";
            } else {
                // Remover saldo (verificar se tem saldo suficiente)
                if ($adminWallet->available_balance < $amount) {
                    DB::rollBack();
                    return back()->withErrors(['error' => 'Saldo insuficiente. Saldo disponível: R$ ' . number_format($adminWallet->available_balance, 2, ',', '.')]);
                }
                
                $adminWallet->addDebit(
                    $amount,
                    'manual_adjustment',
                    $description,
                    [
                ...
                $message = "R$ " . number_format($amount, 2, ',', '.') . " removido com sucesso!";
            }
            
            DB::commit();
            
            Log::info('Saldo do admin ajustado manualmente', [
                'admin_id' => auth()->id(),
                'type' => $request->type,
                'amount' => $amount,
            ...
            DB::rollBack();
            Log::error('Erro ao ajustar saldo do admin: ' . $e->getMessage());
            return back()->withErrors(['error' => 'Erro ao ajustar saldo: ' . $e->getMessage()]);
        }
    }
}
