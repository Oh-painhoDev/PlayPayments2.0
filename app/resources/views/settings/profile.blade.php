...
@section('page-description', 'Gerencie suas informações pessoais e configurações da conta')
                ...
                <form action="{{ route('settings.profile.update') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        <label for="document" class="block text-sm font-bold text-gray-900 mb-2">
                            {{ $user->isPessoaFisica() ? 'CPF' : 'CNPJ' }}
                        </label>
                        <div class="relative">
                            <input 
                                id="document" 
                                type="text" 
                                value="{{ $user->formatted_document }}"
                                class="w-full px-4 py-3 bg-gray-100 border-2 border-gray-300 rounded-xl text-gray-600 focus:outline-none transition-all duration-200"
                                readonly
                                disabled
                            >
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-2xl p-6 shadow-sm">
                ...
                <div class="space-y-3">
                    ...
                    <div class="bg-white rounded-xl border-2 border-emerald-200 p-3">
                        <span class="text-xs font-bold text-emerald-700 uppercase tracking-wide block mb-1">{{ $user->isPessoaFisica() ? 'CPF' : 'CNPJ' }}</span>
                        <p class="text-gray-900 font-bold">{{ $user->formatted_document }}</p>
                    </div>
