...
@section('title', 'Configurações - API')
@section('page-title', 'Chave Secreta API')
@section('page-description', 'Gerencie sua chave secreta para integração com sistemas externos')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gradient-to-br from-red-50 to-rose-50 border-2 border-red-200 rounded-xl p-4">
                                ...
                                <p class="text-sm text-gray-900 font-semibold">
                                    @if($user->api_secret_created_at && $user->api_secret_created_at instanceof \Carbon\Carbon)
                                        {{ $user->api_secret_created_at->format('d/m/Y H:i') }}
                                    @elseif($user->api_secret_created_at)
                                        {{ \Carbon\Carbon::parse($user->api_secret_created_at)->format('d/m/Y H:i') }}
                                    @else
                                        N/A
                                    @endif
                                </p>
                            ...
                            <div class="bg-gradient-to-br from-red-50 to-rose-50 border-2 border-red-200 rounded-xl p-4">
                                ...
                                <p class="text-sm text-gray-900 font-semibold">
                                    @if($user->api_secret_last_used_at && $user->api_secret_last_used_at instanceof \Carbon\Carbon)
                                        {{ $user->api_secret_last_used_at->format('d/m/Y H:i') }}
                                    @elseif($user->api_secret_last_used_at)
                                        {{ \Carbon\Carbon::parse($user->api_secret_last_used_at)->format('d/m/Y H:i') }}
                                    @else
                                        Nunca utilizada
                                    @endif
                                </p>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-6">
                            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-4">
                                ...
                                <p class="text-sm text-gray-900 font-semibold">
                                    @if($user->api_public_key_created_at && $user->api_public_key_created_at instanceof \Carbon\Carbon)
                                        {{ $user->api_public_key_created_at->format('d/m/Y H:i') }}
                                    @elseif($user->api_public_key_created_at)
                                        {{ \Carbon\Carbon::parse($user->api_public_key_created_at)->format('d/m/Y H:i') }}
                                    @else
                                        N/A
                                    @endif
                                </p>
                ...
                <div class="bg-white rounded-2xl border-2 border-gray-200 p-6 shadow-sm">
                    <div class="flex items-start">
                        ...
                        <div class="ml-4 flex-1">
                            ...
                            <form action="{{ route('settings.api.regenerate') }}" method="POST" onsubmit="return confirm('Tem certeza que deseja regenerar a chave secreta? A chave atual será invalidada!')">
                                @csrf
                                <button 
                                    type="submit" 
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-2xl p-6 shadow-sm">
                ...
                <div class="space-y-3">
                    <a href="{{ route('settings.api.docs') }}" class="flex items-center p-3 bg-white rounded-xl border-2 border-emerald-200 hover:border-emerald-300 transition-all group">
                    ...
                    <a href="{{ route('settings.api.docs') }}#endpoints" class="flex items-center p-3 bg-white rounded-xl border-2 border-emerald-200 hover:border-emerald-300 transition-all group">
                        <svg class="w-5 h-5 text-emerald-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('settings.api.docs') }}#exemplos" class="flex items-center p-3 bg-white rounded-xl border-2 border-emerald-200 hover:border-emerald-300 transition-all group">
                        <svg class="w-5 h-5 text-emerald-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-white rounded-xl">
                        ...
                        <span class="text-gray-900 text-sm font-bold">
                            {{ $user->api_secret ? 'Ativa' : 'Não gerada' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-white rounded-xl">
                        ...
                        <span class="text-gray-900 text-sm font-bold">
                            {{ $user->api_public_key ? 'Ativa' : 'Não gerada' }}
                        </span>
                    </div>
                </div>
            </div>
