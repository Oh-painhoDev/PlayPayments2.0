...
@section('page-description', 'Conecte e automatize notificações em tempo real')
                ...
                <ul class="space-y-4" id="webhooksList">
                    ...
                    @else
                        @foreach($webhooks as $webhook)
                            <li class="webhook-item bg-[#1f1f1f] rounded-lg p-4 border border-[#2d2d2d] hover:border-[#D4AF37]/50 transition-colors" data-description="{{ strtolower($webhook->description ?? '') }}" data-url="{{ strtolower($webhook->url) }}">
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-white text-sm font-semibold mb-1">{{ $webhook->description ?: 'Webhook #' . $webhook->id }}</h3>
                                        <p class="text-[#AAAAAA] text-xs font-mono truncate max-w-md mb-2">{{ $webhook->url }}</p>
                                            ...
                                            <div class="flex items-center justify-between gap-2">
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <div class="flex items-center gap-2">
                                                        <code id="secret-{{ $webhook->id }}" class="text-xs font-mono text-white break-all">{{ $webhook->masked_secret }}</code>
                                                        <button 
                                                            onclick="toggleSecret({{ $webhook->id }}, {{ json_encode($webhook->secret) }}, {{ json_encode($webhook->masked_secret) }})"
                                                        ...
                                                        <button 
                                                            onclick="copySecret({{ json_encode($webhook->secret) }}, {{ $webhook->id }})"
                                        ...
                                        <div class="flex items-center gap-2 mt-2">
                                            <span class="text-xs text-[#707070]">{{ $webhook->formatted_events }}</span>
                                    ...
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        <button 
                                            onclick="openEditWebhookModal({{ $webhook->id }}, '{{ addslashes($webhook->url) }}', '{{ addslashes($webhook->description ?? '') }}', {{ json_encode($webhook->events) }}, {{ $webhook->is_active ? 'true' : 'false' }})"
                ...
                @if($webhooks->hasPages())
                    <div class="flex items-center justify-center w-full mt-6">
                        <div class="flex items-center gap-2">
                            ...
                            @else
                                <a href="{{ $webhooks->previousPageUrl() }}" class="justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-8 px-3 py-1 rounded-md flex items-center gap-1.5 bg-[#1f1f1f] hover:bg-[#2a2a2a] text-[#aaaaaa] hover:text-white button-custom">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-left h-3 w-3">
                                        <path d="m15 18-6-6 6-6"></path>
                                    </svg>
                                    <span class="text-[12px] font-semibold font-['Manrope'] tracking-[-0.24px]">Anterior</span>
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                @if($start > 1)
                                    <a href="{{ $webhooks->url(1) }}" class="w-8 h-8 bg-[#1f1f1f] hover:bg-[#2a2a2a] rounded flex items-center justify-center text-[#aaaaaa] hover:text-white transition-colors">
                                ...
                                @foreach(range($start, $end) as $page)
                                    ...
                                    @else
                                        <a href="{{ $webhooks->url($page) }}" class="w-8 h-8 bg-[#1f1f1f] hover:bg-[#2a2a2a] rounded flex items-center justify-center text-[#aaaaaa] hover:text-white transition-colors">
                                            <span class="text-[12px] font-semibold font-['Manrope'] tracking-[-0.24px]">{{ $page }}</span>
                                        </a>
                                    @endif
                                @endforeach
                                ...
                                @if($end < $lastPage)
                                    @if($end < $lastPage - 1)
                                        <span class="text-[#707070] px-2">...</span>
                                    @endif
                                    <a href="{{ $webhooks->url($lastPage) }}" class="w-8 h-8 bg-[#1f1f1f] hover:bg-[#2a2a2a] rounded flex items-center justify-center text-[#aaaaaa] hover:text-white transition-colors">
                                        <span class="text-[12px] font-semibold font-['Manrope'] tracking-[-0.24px]">{{ $lastPage }}</span>
                                    </a>
                                @endif
                            </div>
                            
                            @if($webhooks->hasMorePages())
                                <a href="{{ $webhooks->nextPageUrl() }}" class="justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-8 px-3 py-1 rounded-md flex items-center gap-1.5 bg-[#1f1f1f] hover:bg-[#2a2a2a] text-[#aaaaaa] hover:text-white button-custom">
...
<div id="addWebhookModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="flex flex-col gap-4 w-full">
        ...
        <form action="{{ route('webhooks.store') }}" method="POST" class="flex flex-col gap-2.5 w-full" onsubmit="return validateWebhookForm(this)">
            @csrf
            ...
            <div class="flex flex-col gap-2.5 w-full">
                ...
                <div class="flex items-center justify-between w-full">
                    ...
                    <label class="toggle-switch flex h-5 w-10 items-center rounded-[100px] p-[2px] transition-all cursor-pointer bg-[#D4AF37] justify-end" data-event="transaction.paid">
                ...
                <div class="flex items-center justify-between w-full">
                    ...
                    <label class="toggle-switch flex h-5 w-10 items-center rounded-[100px] p-[2px] transition-all cursor-pointer bg-[#D4AF37] justify-end" data-event="transaction.created">
...
function updateToggle(checkbox) {
    ...
    if (checkbox.checked) {
        ...
        label.classList.add('bg-[#D4AF37]', 'justify-end');
        toggle.classList.remove('bg-[#161616]');
        toggle.classList.add('bg-white');
    } else {
        label.classList.remove('bg-[#D4AF37]', 'justify-end');
...
function openEditWebhookModal(id, url, description, events, isActive) {
    ...
    form.action = '{{ route("webhooks.update", ":id") }}'.replace(':id', id);
    ...
    eventValues.forEach(eventValue => {
        ...
        eventsHTML += `
            <div class="flex items-center justify-between w-full">
                ...
                <label class="toggle-switch flex h-5 w-10 items-center rounded-[100px] p-[2px] transition-all cursor-pointer ${isChecked ? 'bg-[#D4AF37] justify-end' : 'bg-[#1F1F1F] justify-start'}" data-event="${eventValue}">
...
function confirmDeleteWebhook(id) {
    if (confirm('Tem certeza que deseja excluir este webhook?')) {
        ...
        form.method = 'POST';
        form.action = '{{ route("webhooks.destroy", ":id") }}'.replace(':id', id);
