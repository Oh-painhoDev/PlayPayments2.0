...
@section('title', 'Configuração de Retentativa')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        <form action="{{ route('admin.retry.update') }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input 
                            ...
                            name="is_enabled" 
                            value="1"
                            {{ ($retryConfig && $retryConfig->is_enabled) ? 'checked' : '' }}
                            class="sr-only peer"
                        >
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach($gateways as $gateway)
                            <option value="{{ $gateway->id }}" {{ ($retryConfig && $retryConfig->retry_gateway_id == $gateway->id) ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    >{{ $retryConfig->description ?? '' }}</textarea>
                </div>
    ...
    <div class="mt-6 bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-3">
            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                ...
                <span class="px-3 py-1 rounded-full text-sm font-medium {{ ($retryConfig && $retryConfig->is_enabled) ? 'bg-green-500/10 text-green-700' : 'bg-gray-500/10 text-gray-700' }}">
                    {{ ($retryConfig && $retryConfig->is_enabled) ? 'Ativada' : 'Desativada' }}
                </span>
            </div>
            ...
            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                ...
                <span class="text-gray-900 font-medium">
                    {{ ($retryConfig && $retryConfig->retryGateway) ? $retryConfig->retryGateway->name : 'Nenhum' }}
                </span>
            </div>
            ...
            <div class="flex items-center justify-between py-2">
                ...
                <span class="text-gray-900 font-medium">
                    {{ $retryConfig && $retryConfig->updated_at ? $retryConfig->updated_at->format('d/m/Y H:i') : 'Nunca' }}
                </span>
            </div>
        </div>
    </div>
