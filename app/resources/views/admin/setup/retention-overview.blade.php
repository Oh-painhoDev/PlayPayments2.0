...
@section('title', 'Visão Geral de Retenções')
@section('page-title', 'Visão Geral de Retenções')
@section('page-description', 'Visualize todas as configurações de retenção e transações retidas')
...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['users_with_retention']) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['individual_configs']) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['total_retained']) }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">R$ {{ number_format($stats['total_amount'], 2, ',', '.') }}</p>
        ...
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-lg font-bold text-gray-900">{{ number_format($stats['active_configs']) }}</p>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.users.index') }}" class="bg-blue-600 hover:bg-blue-700 text-gray-900 px-4 py-2 rounded-lg text-sm transition-colors">
                    Gerenciar Usuários
                </a>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($usersWithRetention as $userData)
                        @php
                            $user = $userData['user'];
                        ...
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-gray-900 text-sm">
                                    {{ $config->quantity_cycle }} de {{ $config->quantity_cycle }}
                                </div>
                                <div class="text-gray-600 text-xs">
                                    Reter {{ $config->quantity_retained }} a cada {{ $config->quantity_cycle }} pagas
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900 text-sm">{{ $progress['paid_count'] }}/{{ $config->quantity_cycle }}</div>
                                <div class="w-full bg-gray-800 rounded-full h-1.5 mt-1">
                                    <div class="bg-blue-600 h-1.5 rounded-full" style="width: {{ $progress['cycle_progress'] }}%"></div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900 text-sm">{{ $progress['retained_count'] }}/{{ $config->quantity_retained }}</div>
                                <div class="w-full bg-gray-800 rounded-full h-1.5 mt-1">
                                    <div class="bg-purple-600 h-1.5 rounded-full" style="width: {{ $progress['retained_progress'] }}%"></div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900 font-medium">{{ $userData['total_retained'] }}</div>
                                <div class="text-gray-600 text-xs">transações</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-green-600 font-medium">R$ {{ number_format($userData['total_retained_amount'], 2, ',', '.') }}</div>
                                <div class="text-gray-600 text-xs">valor líquido</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900 font-medium">{{ $user->formatted_wallet_balance }}</div>
                                <div class="text-gray-600 text-xs">saldo atual</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.users.show', $user) }}" class="text-blue-600 hover:text-blue-700 text-sm">
                                        Ver Perfil
                                    </a>
                                    <a href="{{ route('admin.users.retention', $user) }}" class="text-purple-400 hover:text-purple-300 text-sm">
                                        Configurar
                                    </a>
                                    <form action="{{ route('admin.users.retention.reset', $user) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja reiniciar o ciclo deste usuário?');">
                                        @csrf
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    <select name="user_id" class="w-full px-3 py-2 bg-white border border-gray-600 rounded-lg text-gray-900 text-sm">
                        ...
                        @foreach($usersWithRetention as $userData)
                            <option value="{{ $userData['user']->id }}" {{ request('user_id') == $userData['user']->id ? 'selected' : '' }}>
                                {{ $userData['user']->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($retainedTransactions as $transaction)
                        <tr class="hover:bg-gray-50/50">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm">Líquido: {{ $transaction->formatted_net_amount }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    ...
                                    <form action="{{ route('admin.setup.retained-sales.return', $transaction->transaction_id) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja devolver esta venda para o usuário?');">
                                        @csrf
                                        <button type="submit" class="text-green-600 hover:text-green-700 text-sm">
                                            Devolver
                                        </button>
        ...
        <!-- Pagination -->
        @if($retainedTransactions->hasPages())
            <div class="px-6 py-3 border-t border-gray-200 mt-4">
                {{ $retainedTransactions->appends(request()->query())->links() }}
            </div>
        @endif
    </div>
</div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    <div class="bg-gray-800 rounded-lg p-4 border border-gray-300">
                        ...
                        <p class="text-gray-700 text-sm">{{ $globalConfig ? $globalConfig->quantity_cycle . ' de ' . $globalConfig->quantity_cycle : 'Não configurado' }}</p>
                        <p class="text-gray-600 text-xs">{{ $globalConfig ? 'Reter ' . $globalConfig->quantity_retained . ' a cada ' . $globalConfig->quantity_cycle . ' pagas' : '' }}</p>
                    </div>
                    ...
                    <div class="bg-gray-800 rounded-lg p-4 border border-gray-300">
                        ...
                        <p class="text-gray-700 text-sm">
                            @if($globalConfig)
                                @if($globalConfig->selection_mode === 'all')
                                    Usuários Selecionados
                                @elseif($globalConfig->selection_mode === 'selected')
                                    Usuários Selecionados
                                @else
                    ...
                    <div class="bg-gray-800 rounded-lg p-4 border border-gray-300">
                        ...
                        <p class="text-gray-700 text-sm">{{ $globalConfig ? count($globalConfig->selected_users ?? []) : 0 }}</p>
                    </div>
                    
                    <div class="bg-gray-800 rounded-lg p-4 border border-gray-300">
                        <h4 class="text-gray-900 font-medium mb-2">Último Reset</h4>
                        <p class="text-gray-700 text-sm">{{ $globalConfig && $globalConfig->last_reset_at ? $globalConfig->last_reset_at->format('d/m/Y H:i') : 'Nunca' }}</p>
                    </div>
                </div>
