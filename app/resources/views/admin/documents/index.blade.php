...
@section('title', 'Solicitações Gateway')
@section('page-title', 'Solicitações Gateway')
@section('page-description', 'Veja a lista de todas as solicitações pendentes para usar a plataforma.')
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($pendingVerifications as $verification)
                        ...
                        <tr class="hover:bg-gray-50/50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center mr-3">
                                        <span class="text-gray-900 font-medium text-sm">{{ substr($user->name, 0, 2) }}</span>
                                    </div>
                                    <div>
                                        <div class="text-gray-900 font-medium">{{ $user->name }}</div>
                                        <div class="text-gray-600 text-sm">{{ $user->formatted_document }}</div>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-gray-900 font-medium">R$ {{ number_format($avgRevenue, 2, ',', '.') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-900">{{ $verification->created_at ? $verification->created_at->format('d/m/Y') : 'N/A' }}</div>
                                <div class="text-gray-600 text-sm">às {{ $verification->created_at ? $verification->created_at->format('H:i') : 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4">
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-gray-50 rounded-lg max-w-md w-full">
            <div class="p-6">
                ...
                <form id="approvalForm" action="{{ route('admin.documents.respond') }}" method="POST" class="space-y-6">
                    @csrf
