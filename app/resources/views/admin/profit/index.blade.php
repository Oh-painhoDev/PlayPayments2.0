...
@section('title', 'Lucro por Empresas')
@section('page-title', 'Lucro por Empresas')
@section('page-description', 'Visualize o lucro gerado por cada empresa cadastrada no sistema')
                        ...
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <p class="text-gray-600 text-xs">{{ number_format($company->total_transactions ?? 0) }} transações</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-gray-900 font-medium">R$ {{ number_format($company->total_fees ?? 0, 2, ',', '.') }}</p>
                                    <p class="text-gray-600 text-xs">{{ number_format(($company->total_fees / max($company->total_sales, 0.01)) * 100, 2) }}%</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-green-600 font-medium">R$ {{ number_format($company->gateway_fees ?? 0, 2, ',', '.') }}</p>
                                    <p class="text-gray-600 text-xs">{{ number_format(($company->gateway_fees / max($company->total_sales, 0.01)) * 100, 2) }}%</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-emerald-400 font-medium">R$ {{ number_format($company->profit ?? 0, 2, ',', '.') }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    <p class="text-gray-900 font-medium">{{ number_format(($company->profit / max($company->total_fees, 0.01)) * 100, 2) }}%</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.profit.show', $company) }}" class="text-blue-600 hover:text-blue-700 text-sm">
                                        Ver detalhes
                                    </a>
