...
@section('title', 'Configuração de Retenção Individual')
@section('page-title', 'Configuração de Retenção: ' . $user->name)
@section('page-description', 'Configure a retenção individual para este usuário')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('admin.users.show', $user) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm">
                    Voltar
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <form action="{{ route('admin.users.retention.update', $user) }}" method="POST" class="space-y-6">
                @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                name="is_active" 
                                value="1" 
                                {{ old('is_active', $retentionConfig->is_active) ? 'checked' : '' }}
                            ...
                            <div class="space-y-3">
                                <div class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-emerald-400 transition-colors cursor-pointer">
                                    <input 
                                        ...
                                        name="retention_type" 
                                        value="1" 
                                        {{ old('retention_type', $user->retention_type ?? 1) == 1 ? 'checked' : '' }}
                                ...
                                <div class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-emerald-400 transition-colors cursor-pointer">
                                    <input 
                                        ...
                                        name="retention_type" 
                                        value="2" 
                                        {{ old('retention_type', $user->retention_type ?? 1) == 2 ? 'checked' : '' }}
                                        class="h-4 w-4 mt-1 text-emerald-600 focus:ring-emerald-500 border-gray-300"
                                    >
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                max="100"
                                required 
                                value="{{ old('quantity_cycle', $retentionConfig->quantity_cycle) }}"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                max="50"
                                required 
                                value="{{ old('quantity_retained', $retentionConfig->quantity_retained) }}"
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                                placeholder="1"
                            >
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-sm text-gray-900">{{ $currentCycleCount }} / {{ $retentionConfig->quantity_cycle }}</span>
                        </div>
                        <div class="w-full bg-gray-800 rounded-full h-2.5">
                            <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ min(100, ($currentCycleCount / max(1, $retentionConfig->quantity_cycle)) * 100) }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-sm text-gray-900">{{ $currentRetainedCount }} / {{ $retentionConfig->quantity_retained }}</span>
                        </div>
                        <div class="w-full bg-gray-800 rounded-full h-2.5">
                            <div class="bg-purple-600 h-2.5 rounded-full" style="width: {{ min(100, ($currentRetainedCount / max(1, $retentionConfig->quantity_retained)) * 100) }}%"></div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-gray-900 text-sm">{{ $totalRetained }} transações</span>
                    </div>
                    
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 text-sm">Valor Total Retido:</span>
                        <span class="text-gray-900 text-sm">R$ {{ number_format($totalRetainedAmount, 2, ',', '.') }}</span>
                    </div>
                    
                    @if($retentionConfig->last_reset_at)
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 text-sm">Último Reset:</span>
                        <span class="text-gray-900 text-sm">{{ $retentionConfig->last_reset_at->format('d/m/Y H:i') }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900 text-sm">{{ $user->formatted_wallet_balance }}</span>
                    </div>
                </div>
            </div>
        </div>
