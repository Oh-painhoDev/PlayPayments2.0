...
@section('title', 'Templates de Infrações')
@section('page-title', 'Templates de Infrações')
...
<div class="p-6 space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 rounded-2xl p-8 text-white shadow-xl">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.setup.dispute-templates.create') }}" class="bg-white hover:bg-gray-100 text-purple-700 px-6 py-3 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl flex items-center">
        ...
        <form method="GET" class="flex flex-wrap gap-4">
            ...
            <div class="min-w-[180px]">
                <select name="dispute_type" 
                    ...
                    <option value="fraud" {{ request('dispute_type') == 'fraud' ? 'selected' : '' }}>Fraude</option>
                    <option value="unauthorized" {{ request('dispute_type') == 'unauthorized' ? 'selected' : '' }}>Não Autorizada</option>
                    <option value="not_received" {{ request('dispute_type') == 'not_received' ? 'selected' : '' }}>Não Recebido</option>
                    <option value="defective" {{ request('dispute_type') == 'defective' ? 'selected' : '' }}>Defeituoso</option>
                    <option value="other" {{ request('dispute_type') == 'other' ? 'selected' : '' }}>Outro</option>
                </select>
            </div>
            ...
            <div class="min-w-[150px]">
                <select name="risk_level" 
                    ...
                    <option value="MED" {{ request('risk_level') == 'MED' ? 'selected' : '' }}>Médio</option>
            ...
            @if(request()->hasAny(['search', 'dispute_type', 'risk_level']))
            <a href="{{ route('admin.setup.dispute-templates.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-xl font-semibold transition-all">
                Limpar
            </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-2xl border-2 border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $riskColors[$template->risk_level] ?? 'bg-gray-100 text-gray-800' }}">
                                    {{ $template->getRiskLevelLabel() }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <button onclick="toggleStatus({{ $template->id }}, this)" 
                                    class="px-3 py-1 text-xs font-semibold rounded-full {{ $template->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="{{ route('admin.setup.dispute-templates.edit', $template) }}" 
                                ...
                                <form action="{{ route('admin.setup.dispute-templates.destroy', $template) }}" method="POST" class="inline-block" 
        ...
        @else
            <div class="p-12 text-center">
                ...
                <div class="mt-6">
                    <a href="{{ route('admin.setup.dispute-templates.create') }}" 
