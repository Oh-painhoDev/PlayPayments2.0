...
@section('title', 'Multi-Liquidante')
...
<div class="flex-1 overflow-auto">
    <div class="p-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6">
                <form action="{{ route('admin.multi-gateway.update') }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" id="is_enabled" name="is_enabled" value="1" {{ $config->is_enabled ? 'checked' : '' }} class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6" id="mode-section">
                            <div>
                                ...
                                <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" name="mode" id="mode" {{ !$config->is_enabled ? 'disabled' : '' }}>
                                    <option value="global" {{ $config->mode === 'global' ? 'selected' : '' }}>Global (todos os usuários)</option>
                                    <option value="specific_users" {{ $config->mode === 'specific_users' ? 'selected' : '' }}>Usuários Específicos</option>
                                    <option value="all_except" {{ $config->mode === 'all_except' ? 'selected' : '' }}>Todos Exceto...</option>
                                </select>
                            </div>
                            
                            <div id="users-section" style="display: {{ in_array($config->mode, ['specific_users', 'all_except']) ? 'block' : 'none' }}">
                                <label class="block text-sm font-semibold text-gray-900 mb-2">Selecionar Usuários</label>
                                <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" name="selected_users[]" id="selected_users" multiple size="5" {{ !$config->is_enabled ? 'disabled' : '' }}>
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ in_array($user->id, $config->selected_users ?? []) ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @foreach($gateways as $gateway)
                                    <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                                        <input type="checkbox" name="selected_gateways[]" value="{{ $gateway->id }}" id="gateway_{{ $gateway->id }}" {{ in_array($gateway->id, $config->selected_gateways ?? []) ? 'checked' : '' }} {{ !$config->is_enabled ? 'disabled' : '' }} class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500">
                                        <span class="ml-2 text-sm text-gray-900">{{ $gateway->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
