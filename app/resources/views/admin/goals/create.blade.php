...
@section('content')
<div class="p-6 space-y-8 max-w-[1000px] mx-auto animate-in fade-in duration-700" x-data="{ period: '{{ old('period', 'monthly') }}' }">
    <!-- Header Card -->
    <div class="relative overflow-hidden group rounded-3xl border border-white/5 shadow-2xl">
        <div class="absolute inset-0 bg-gradient-to-r from-emerald-500/10 via-purple-500/10 to-transparent"></div>
        <div class="relative bg-[#121212]/80 backdrop-blur-xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.3)] group-hover:scale-110 transition-transform duration-500">
                    <svg class="w-8 h-8 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                </div>
                <div>
                    <h1 class="text-3xl font-black text-white tracking-tight">Criar Nova Meta</h1>
                    <p class="text-gray-400 font-medium tracking-tight">Expandindo as fronteiras de faturamento da plataforma</p>
                </div>
            </div>
    ...
    <div class="bg-[#121212]/60 backdrop-blur-xl border border-white/5 rounded-3xl shadow-2xl overflow-hidden">
        <form action="{{ route('admin.goals.store') }}" method="POST" class="p-8 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <select id="user_id" name="user_id" class="w-full h-14 px-5 bg-white/[0.03] border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/30 focus:border-[#D4AF37] transition-all cursor-pointer">
                        <option value="global" {{ old('user_id') === 'global' || old('user_id') === null ? 'selected' : '' }} class="bg-[#1a1a1a]">🌍 Meta Global (Todos)</option>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id') == $user->id ? 'selected' : '' }} class="bg-[#1a1a1a]">👤 {{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="type" name="type" required class="w-full h-14 px-5 bg-white/[0.03] border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/30 focus:border-[#D4AF37] transition-all cursor-pointer">
                        <option value="" class="bg-[#1a1a1a]">Selecione a Métrica</option>
                        @foreach($types as $type)
                            <option value="{{ $type }}" {{ old('type') == $type ? 'selected' : '' }} class="bg-[#1a1a1a]">{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="target_value" name="target_value" value="{{ old('target_value') }}" step="0.01" min="0" required
            ...
            <div class="space-y-6 bg-white/[0.02] p-6 rounded-3xl border border-white/5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2 group">
                        ...
                        <input type="number" id="display_order" name="display_order" value="{{ old('display_order') }}" min="1"
                ...
                <div x-show="period === 'custom'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-4" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date') }}" :required="period === 'custom'"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date') }}" :required="period === 'custom'"
                            class="w-full h-12 px-5 bg-white/[0.03] border border-white/10 rounded-xl text-white focus:outline-none focus:ring-[#D4AF37] transition-all invert brightness-90">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="3" class="w-full p-5 bg-white/[0.03] border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/30 transition-all resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="relative p-8 bg-gradient-to-br from-[#D4AF37]/5 to-transparent border border-[#D4AF37]/10 rounded-[2rem] space-y-8 overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <select id="reward_type" name="reward_type" class="w-full h-14 px-5 bg-white/5 border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-[#D4AF37] cursor-pointer">
                            ...
                            <option value="cash" {{ old('reward_type') == 'cash' ? 'selected' : '' }} class="bg-[#1a1a1a]">Dinheiro Base (Wallet)</option>
                            <option value="bonus" {{ old('reward_type') == 'bonus' ? 'selected' : '' }} class="bg-[#1a1a1a]">Bônus de Volume</option>
                            <option value="discount" {{ old('reward_type') == 'discount' ? 'selected' : '' }} class="bg-[#1a1a1a]">Desconto Operacional</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <span class="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 font-bold">R$</span>
                            <input type="number" id="reward_value" name="reward_value" value="{{ old('reward_value') }}" step="0.01" min="0"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="reward_description" name="reward_description" value="{{ old('reward_description') }}"
                ...
                <div class="flex flex-col md:flex-row gap-8 items-center pt-4">
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" id="auto_reward" name="auto_reward" value="1" {{ old('auto_reward') ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex flex-col md:flex-row justify-end gap-4 pt-8">
                <a href="{{ route('admin.goals.index') }}" class="px-10 py-4 bg-white/5 hover:bg-white/10 text-gray-400 font-bold rounded-2xl border border-white/10 transition-all text-center">Cancelar Meta</a>
                <button type="submit" class="px-12 py-4 bg-emerald-500 hover:bg-emerald-600 text-black font-black uppercase tracking-widest rounded-2xl transition-all shadow-[0_10px_30px_rgba(16,185,129,0.2)]">Ativar Nova Diretriz</button>
            </div>
        </form>
    </div>
</div>
