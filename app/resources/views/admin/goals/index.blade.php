...
<div class="p-6 space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 rounded-2xl p-8 text-white shadow-xl">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.goals.create') }}" class="bg-white hover:bg-gray-100 text-purple-700 px-6 py-3 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl flex items-center">
    ...
    <div class="bg-white rounded-xl border-2 border-gray-200 p-4">
        <form method="GET" action="{{ route('admin.goals.index') }}" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <select name="user_id" class="w-full px-4 py-2 bg-gray-50 border-2 border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    ...
                    <option value="global" {{ request('user_id') === 'global' ? 'selected' : '' }}>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.goals.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-xl font-semibold transition-all">
                    Limpar
                </a>
            </div>
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($goals as $goal)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-bold text-gray-900">{{ $goal->name }}</div>
                                @if($goal->description)
                                    <div class="text-xs text-gray-500">{{ Str::limit($goal->description, 50) }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($goal->user)
                                    <div class="text-sm font-bold text-gray-900">{{ $goal->user->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $goal->user->email }}</div>
                                @else
                                ...
                                @if($percentage !== null)
                                    <div class="flex items-center gap-2">
                                        <div class="flex-1 bg-gray-200 rounded-full h-2 min-w-[100px]">
                                            <div class="bg-purple-600 h-2 rounded-full transition-all duration-500" style="width: {{ min(100, $percentage) }}%;"></div>
                                        </div>
                                ...
                                @if($goal->reward_type && $goal->reward_value)
                                    <div class="text-sm font-bold text-green-600">
                                        @if($goal->reward_type === 'cash')
                                            R$ {{ number_format($goal->reward_value, 2, ',', '.') }}
                                        @else
                                            {{ ucfirst($goal->reward_type) }}
                                        @endif
                                    </div>
                                    @if($goal->auto_reward)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form method="POST" action="{{ route('admin.goals.toggle-status', $goal) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="px-3 py-1 text-xs font-semibold rounded-full transition-all {{ $goal->is_active ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}">
                                ...
                                <div class="flex items-center gap-2">
                                    @if($goal->reward_type && $goal->reward_value && ($percentage !== null && $percentage >= 100))
                                        @if($goal->user)
                                            ...
                                            @endphp
                                            @if(!$hasAchieved)
                                                <form method="POST" action="{{ route('admin.goals.reward-user', $goal) }}" class="inline" onsubmit="return confirm('Tem certeza que deseja premiar o usuário {{ $goal->user->name }}? O prêmio de R$ {{ number_format($goal->reward_value, 2, ',', '.') }} será creditado na wallet.');">
                                                    @csrf
                                        ...
                                        @endif
                                    @endif
                                    <a href="{{ route('admin.goals.edit', $goal) }}" class="text-purple-600 hover:text-purple-900 transition-colors" title="Editar">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.goals.destroy', $goal) }}" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta meta?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <a href="{{ route('admin.goals.create') }}" class="inline-block mt-4 bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-xl font-semibold transition-all">
                                    Criar Meta
                                </a>
                            </td>
                        </tr>
