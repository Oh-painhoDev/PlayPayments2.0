...
/*
|--------------------------------------------------------------------------
| App Subdomain Routes - app.playpayments.com
|--------------------------------------------------------------------------
|
| Todas as rotas web para o subdomínio app.playpayments.com
| Este arquivo contém TODAS as rotas web que devem funcionar apenas no subdomínio APP
|
| IMPORTANTE: Configure o DNS para app.playpayments.com apontar para o mesmo IP do servidor
|
*/
...
// Lista de hosts do subdomínio APP
$appHosts = [
    'app.playpayments.com',
    'app.' . str_replace('www.', '', parse_url(config('app.url'), PHP_URL_HOST) ?? 'localhost'),
];

// Para cada host do subdomínio APP, criar as rotas
foreach ($appHosts as $appHost) {
    Route::domain($appHost)->group(function () {
        
        // Redirect root to acessar
        Route::get('/', function () {
        ...
        Route::middleware('guest')->group(function () {
            // Login (acessar)
            Route::get('/acessar', [AuthController::class, 'showLogin'])->name('login');
            Route::post('/acessar', [AuthController::class, 'login'])->name('login.post');
            
            // Redirect /login para /acessar (compatibilidade)
            Route::get('/login', function () {
                return redirect('/acessar');
            });
            
            // Register (usando AuthController simplificado - apenas 4 campos básicos)
        ...
        Route::middleware(['auth', CheckUserBlocked::class, 'onboarding.gate'])->group(function () {
            ...
            Route::middleware([DocumentVerificationMiddleware::class])->group(function () {
                ...
                // Settings Routes (except documents, password, api-keys, webhooks)
                Route::prefix('settings')->name('settings.')->group(function () {
